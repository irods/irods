...
namespace
{
    ...
    {
        if (!_rei || !_rei->rsComm) {
            rodsLog(LOG_ERROR, "Rule execution info pointer or connection pointer is null");
        ...
        try {
            ...
            if (!out) {
                rodsLog(LOG_ERROR, "Cannot open data object for writing [path => %s]", _path);
...
{
    if (!writeId) {
        rodsLog(LOG_ERROR, "Output target is null");
        return 0;
    }

    if (std::strcmp(writeId, "serverLog") == 0) {
        rodsLog(LOG_NOTICE, "writeString: %s", writeStr);
        return 0;
    }

    if (!rei->rsComm) {
        rodsLog(LOG_ERROR, "Connection object is null");
        return 0;
    }
    ...
    if (strcmp(writeId, "stdout") == 0 || strcmp(writeId, "stderr") == 0) {
        ...
        rodsLog(LOG_ERROR, "Invalid output target [output target => %s]", writeId);
    }
