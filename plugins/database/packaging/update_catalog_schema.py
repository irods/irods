...
def get_current_schema_version( cfg ):
    ...
    if ( "relation \"r_grid_configuration\" does not exist" in result[2] ):
        ...
        current_schema_version = int(result[1].split("\n")[2].strip())
    if DEBUG: print "current_schema_version: %d" % current_schema_version
...
def get_target_schema_version():
    ...
    for line in lines:
        ...
        if ( name.strip() == "CATALOG_SCHEMA_VERSION" ):
            if DEBUG: print "CATALOG_SCHEMA_VERSION found in %s..." % version_file
            target_schema_version = int(value.strip())
            if DEBUG: print "CATALOG_SCHEMA_VERSION: %d" % target_schema_version
    # return it
    if DEBUG: print "target_schema_version: %d" % target_schema_version
    return target_schema_version

def update_schema_version( cfg, version ):
    if DEBUG: print "Updating schema_version..."
    # update the database
    ...
    if ( result[2] != "" ):
        ...
        return
    # success
    if DEBUG: print "SUCCESS, updated to schema_version %d" % version
...
def update_database_to_latest_version():
    ...
    if ( current_schema_version > target_schema_version ) :
        print "Catalog Schema Version is from the future (current=%d > target=%d)." % ( current_schema_version, target_schema_version )
        return
    if ( current_schema_version == target_schema_version ) :
        print "Catalog Schema Version is already up to date (version=%d)." % target_schema_version
        return
    # read possible update sql files
    foundfiles = get_update_files(current_schema_version, cfg.values[ 'catalog_database_type' ])
    if DEBUG: print "files: %s" % foundfiles
    updatefiles = {}
    for f in foundfiles:
        version = int(f.split("/")[-1].split(".")[0])
        updatefiles[version] = f
    if DEBUG: print "updatefiles: %s" % updatefiles
    # check all necessary update files exist
    for version in range(current_schema_version+1,target_schema_version+1):
        if DEBUG: print "checking... %d" % version
        if version not in updatefiles.keys():
            print "ERROR: SQL Update File Not Found for Schema Version %d" % version
            return
    # run each update sql file
    for version in range(current_schema_version+1,target_schema_version+1):
        print "Updating to Catalog Schema... %d" % version
        if DEBUG: print "running: %s" % updatefiles[version]
        result = cfg.exec_sql_file(updatefiles[version])
        if ( result[2] != "" ):
            print "ERROR: SQL did not complete..."
            print result[2]
            return
        if DEBUG: print "sql result..."
        if DEBUG: print result
