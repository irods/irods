...
namespace
{
    auto is_client_to_agent_connection() -> bool
    {
        try {
            ...
            rodsLog(LOG_DEBUG, "Connection is not an agent-to-agent connection");
        }
...
namespace irods
{
    ...
    auto client_api_whitelist::add(int api_number) -> void
    {
        ...
        try {
            ...
            const auto ec = SYS_INTERNAL_ERR;
            const auto* msg = "Could not add API number to whitelist [error_code => %d, exception => %s]";
            rodsLog(LOG_ERROR, msg, ec, e.what());
        }
    }
} // namespace irods
