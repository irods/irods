...
#include "msParam.h"
#include "objInfo.h" // For StrArray
...
namespace
{
    ...
    auto gather_json_names(const json& _json) -> std::tuple<std::vector<std::string>, int>
    {
        ...
        rodsLog(LOG_DEBUG, fmt::format("names = [{}]", fmt::format("{}", fmt::join(names, ", "))).c_str());
    ...
    auto msi_impl(MsParam* _json_handle, MsParam* _json_pointer, MsParam* _names, ruleExecInfo_t* _rei) -> int
    {
        ...
        try {
            ...
            rodsLog(LOG_ERROR, "Caught exception while fetching JSON property names. [%s]", e.client_display_what());
            return static_cast<int>(e.code());
        }
        catch (const boost::bad_any_cast& e) {
            rodsLog(LOG_ERROR, "Caught exception while fetching JSON property names. [%s]", e.what());
