...
extern "C" {
    ...
    eirods::error pass_thru_file_create_plugin( 
        ...
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_create_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_create_plugin - failed getting the first child resource pointer.", ret);
            } else {
                ret = resc->call( _ctx->comm(), "create", _ctx->fco());
                if(!ret.ok()) {
                    result = PASSMSG("pass_thru_file_create_plugin - failed calling child create.", ret);
                } else {
    ...
    eirods::error pass_thru_file_open_plugin( 
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_open_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_open_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_read_plugin(  
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_read_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_read_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_write_plugin( 
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_write_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_write_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_close_plugin(  
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_close_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_close_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_unlink_plugin(  
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_unlink_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_unlink_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_stat_plugin(  
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_stat_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_stat_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_fstat_plugin(  
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_fstat_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_fstat_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_lseek_plugin(
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_lseek_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_lseek_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_fsync_plugin(  
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_fsync_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_fsync_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_mkdir_plugin(  
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_mkdir_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_mkdir_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_chmod_plugin(  
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_chmod_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_chmod_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_rmdir_plugin( 
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_rmdir_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_rmdir_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_opendir_plugin( 
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_opendir_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_opendir_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_closedir_plugin(  
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_closedir_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_closedir_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_readdir_plugin(  
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_readdir_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_readdir_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_stage_plugin(  
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_stage_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_stage_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_rename_plugin(  
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_rename_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_rename_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_truncate_plugin(  
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_truncate_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_truncate_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_file_getfsfreespace_plugin( 
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_file_getfsfreespace_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_file_getfsfreespace_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_stage_to_cache_plugin( 
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_stage_to_cache_plugin - bad params.", ret);
        } else { 
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_stage_to_cache_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_sync_to_arch_plugin( 
        ...
        ret = pass_thru_check_params( _ctx );
        if(!ret.ok()) {
            result = PASS(false, -1, "pass_thru_sync_to_arch_plugin - bad params.", ret);
        } else {
            eirods::resource_ptr resc;
            ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
            if(!ret.ok()) {
                result = PASS(false, -1, "pass_thru_sync_to_arch_plugin - failed getting the first child resource pointer.", ret);
            } else {
    ...
    eirods::error pass_thru_redirect_plugin( 
        ...
        ret = pass_thru_get_first_chid_resc(_ctx->child_map(), resc);
        if(!ret.ok()) {
            return PASS(false, -1, "pass_thru_redirect_plugin - failed getting the first child resource pointer.", ret);
        } 
