...
#    assertiCmd(s.adminsession,"icd $irodshome" )
    ...
    assertiCmd(s.adminsession, "icp -K -R " + s.testresc + " " +
    ...
    assertiCmd(s.adminsession, "iput -kfR " + irodsdefresource + " -p " +
               phypath + " " + sfile2 + " " + irodshome + "/icmdtest/foo1")
...
# assertiCmd(s.adminsession,"iphybun -R "+s.resgroup+" -Dbzip2 "+irodshome+"/icmdtestbz2" ) # no resgroup anymore
    ...
    assertiCmd(s.adminsession, "itrim -N1 -S " + s.testresc + " -r " +
    ...
    assertiCmd(s.sessions[2], "ireg -KR " + s.testresc + " /tmp/sfile2c " +
               testuser2home + "/foo5", "ERROR", "SYS_NO_PATH_PERMISSION")
...
#    assertiCmd(s.sessions[2],"irm -U "+testuser2home+"/foo5", "ERROR", "CANT_UNREG_IN_VAULT_FILE" )
    ...
    assertiCmd(s.adminsession, "imv " + irodshome + "/icmdtestt/mydirtt/foo1mt " +
               irodshome + "/icmdtestt/mydirtt/foo1mtx")
...
# assertiCmd(s.adminsession,"iphybun -KR "+s.resgroup+" "+irodshome+"/icmdtestp" ) # no resgroup anymore
    ...
    # jmc - resource groups are deprecated - assertiCmd(s.adminsession,"diff
    # -r $mysdir "+dir_w+"/testp", "", "NOANSWER" )
    assertiCmd(s.adminsession, "itrim -rS " + s.testresc + " -N1 " + irodshome + "/icmdtestp", "LIST", "files trimmed")
    # get the name of bundle file
    assertiCmd(s.adminsession, "irm -f --empty " + bunfile)
...
##	assertiCmd(s.adminsession,"irule -F $ruletestfile", "", "", "", "irm "+irodshome+"/icmdtest/foo3" )
    ...
    if os.path.isfile(rsfile):
        os.unlink(rsfile)
    assertiCmd(s.adminsession, "irepl -BvrPT -R " + s.testresc + " --rlock " +
    ...
    assertiCmd(s.adminsession, "iphymv -vrS " + irodsdefresource + " -R " +
               s.testresc + " " + irodshome + "/icmdtest/testz", "LIST", "icmdtest/testz")
    ...
    assert output[1] == "", "diff output was not empty..."
    # test -N0 transfer
    assertiCmd(s.adminsession, "iput -N0 -R " + s.testresc + " " +
    ...
    assertiCmd(s.adminsession, "irepl -BQvrPT -R " + s.testresc + " " +
               irodshome + "/icmdtest/testy", "LIST", "icmdtest/testy")
    assertiCmd(s.adminsession, "itrim -vrS " + irodsdefresource + " -N 1 " +
               irodshome + "/icmdtest/testy", "LIST", "a copy trimmed")
