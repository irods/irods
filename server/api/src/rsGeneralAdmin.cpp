...
namespace
{
    ...
    auto find_first_invalid_char(const std::string& _input, const std::string& _invalid) -> unsigned char
    {
...
int _rsGeneralAdmin(rsComm_t* rsComm, generalAdminInp_t* generalAdminInp)
{
    ...
    if ( strcmp( generalAdminInp->arg0, "modify" ) == 0 ) {
        ...
        if ( strcmp( generalAdminInp->arg1, "resource" ) == 0 ) {
            ...
            if (std::strcmp(generalAdminInp->arg3, "name") == 0) {
                ...
                if (const auto ch = find_first_invalid_char(generalAdminInp->arg4, ";{}"); ch != '\0') {
                    log_api::error(
                        "The character [{}] is not allowed in parent context strings [{}] for resource [{}].",
                        ch,
                        generalAdminInp->arg4,
                        generalAdminInp->arg2);
                    return SYS_INVALID_INPUT_PARAM;
    ...
    if (strcmp(generalAdminInp->arg0, "set_logical_quota") == 0) {
        ...
        if (const auto [ptr, ec] = std::from_chars(
                generalAdminInp->arg3, generalAdminInp->arg3 + std::strlen(generalAdminInp->arg3), nonnegative_checker);
            ec != std::errc{})
        {
        ...
        if (nonnegative_checker < 0) {
            log_api::error("{}: set_logical_quota: Third argument must be nonnegative. Received: [{}]",
                           __func__,
                           nonnegative_checker);
        ...
        if (std::strlen(generalAdminInp->arg1) == 0) {
            log_api::error("{}: set_logical_quota: Empty collection name specified. Received: [{}]",
                           __func__,
                           generalAdminInp->arg1);
            return SYS_INVALID_INPUT_PARAM;
        }
        ...
        const auto zone_collection =
            fmt::format("{}{}{}", irods::get_virtual_path_separator(), local_zone, irods::get_virtual_path_separator());
        ...
        if (std::strncmp(generalAdminInp->arg1,
                         zone_collection.c_str(),
                         std::min(zone_collection.size(),
                                  std::max(zone_collection.size() - 1, std::strlen(generalAdminInp->arg1)))) != 0)
        {
            log_api::error("{}: set_logical_quota: Cannot set quota on non-local zone or root. Attempted setting quota "
                           "on [{}], but zone collection is [{}].",
                           __func__,
                           generalAdminInp->arg1,
                           zone_collection);
        ...
        if (const auto [ptr, ec] = std::from_chars(
                generalAdminInp->arg2, generalAdminInp->arg2 + std::strlen(generalAdminInp->arg2), nonnegative_checker);
            ec != std::errc{})
        {
        ...
        if (nonnegative_checker < 0) {
            log_api::error(
                "{}: set_logical_quota: Second argument must be nonnegative when integer-valued. Received: [{}]",
                __func__,
                nonnegative_checker);
            return USER_INPUT_FORMAT_ERR;
        }
