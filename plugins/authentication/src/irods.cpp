    ...
    class irods_authentication : public irods_auth::authentication_base
    {
      ...
      private:
        ...
        static auto client_auth_with_session_token_op(RcComm& _comm, const nlohmann::json& _request) -> nlohmann::json
        {
            ...
            {
                irods_auth::write_session_token_to_file(resp.at(session_token_kw).get_ref<const std::string&>());
            }
            _comm.loggedIn = 1;
        ...
        static auto server_auth_with_password_op(RsComm& _comm, const nlohmann::json& _request) -> nlohmann::json
        {
            ...
            if (const auto connect_err = getAndConnRcatHostNoLogin(&_comm, PRIMARY_RCAT, zone_name.c_str(), &host);
                connect_err < 0)
            {
            ...
            const auto delete_expired_tokens_input =
            ...
            if (const int delete_token_err =
                    chl_delete_session_tokens(&_comm, delete_expired_tokens_input.dump().c_str());
                delete_token_err < 0)
            {
        ...
        static auto server_auth_with_session_token_op(RsComm& _comm, const nlohmann::json& _request) -> nlohmann::json
        {
            ...
            const auto& session_token = resp.at(session_token_kw).get_ref<const std::string&>();
            int valid = 0;
