...
class Test_Ils(resource_suite.ResourceBase, unittest.TestCase):
    ...
    def test_ils_with_trailing_slash_in_irods_home_config_property__issue_8572(self):
        config = IrodsConfig()
        with lib.file_backed_up(config.client_environment_path):
            ...
            try:
                ...
                self.admin.assert_icommand(['icd'])  # change dir to new home
    ...
    def test_ils_with_multiple_trailing_slashes_in_irods_home_config_property__issue_8572(self):
        config = IrodsConfig()
        with lib.file_backed_up(config.client_environment_path):
            session_env_backup = copy.deepcopy(self.admin.environment_file_contents)
            try:
    ...
    def test_ils_with_trailing_slash_in_irods_cwd_config_property__issue_8572(self):
        # Note that irods_cwd is not normally defined in the environment file.
        config = IrodsConfig()
        with lib.file_backed_up(config.client_environment_path):
            session_env_backup = copy.deepcopy(self.admin.environment_file_contents)
            try:
                ...
                # Force the irods_environment.json.<pid> file to be removed with icd. This will cause iRODS to read
                # the irods_home or irods_cwd that is stored in irods_environment.json.
                # Note that irods_cwd is not normally defined in this file but does take precedence.
    ...
    def test_ils_with_multiple_trailing_slashes_in_irods_cwd_config_property__issue_8572(self):
        # Note that irods_cwd is not normally defined in the environment file.
        config = IrodsConfig()
        with lib.file_backed_up(config.client_environment_path):
            session_env_backup = copy.deepcopy(self.admin.environment_file_contents)
