...
TEST_CASE("Administrators are allowed to use the admin_mode option")
{
    ...
    REQUIRE(ua::client::add_user(conn, test_user).value() == 0);
    ...
    REQUIRE(ua::client::add_group(conn, test_group).value() == 0);
...
TEST_CASE("Non-admin users are not allowed to use the admin_mode option")
{
    ...
    CHECK(ua::client::add_user(conn, test_user).value() == 0);
    REQUIRE(ua::client::set_user_password(conn, test_user, "rods").value() == 0);
