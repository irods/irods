...
namespace
{
    ...
    auto msi_impl(MsParam* _json_handle, MsParam* _json_pointer, MsParam* _result, ruleExecInfo_t* _rei) -> int
    {
        ...
        try {
            ...
            rodsLog(LOG_ERROR, "Caught exception while fetching type of the JSON structure. [%s]", e.client_display_what());
            return static_cast<int>(e.code());
        }
        catch (const boost::bad_any_cast& e) {
            rodsLog(LOG_ERROR, "Caught exception while fetching type of the JSON structure. [%s]", e.what());
