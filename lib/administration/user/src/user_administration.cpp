...
namespace irods::experimental::administration::NAMESPACE_IMPL
{
    ...
    auto add_group(RxComm& _comm, const group& _group) -> void
    {
        ...
        // the client is connected. For 4.3.4 and later, the API can add a "group" and do not need to provide a user
        // "type". Any earlier version only knows how to add a "user" of type "rodsgroup" (which is a group).
