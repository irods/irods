...
class Test_Iadmin(resource_suite.ResourceBase, unittest.TestCase):
    ...
    def test_iadmin_rmuser_removes_associated_data__issue_7778(self):
        ...
        try:
            ...
            self.admin.assert_icommand(['iquest', "SELECT COLL_ACCESS_USER_ID, COLL_ACCESS_NAME WHERE COLL_NAME = '%s' AND COLL_ACCESS_USER_ID %s" % (os.path.join(self.admin.session_collection, test_collection_name), query_string)], 'STDOUT', ['COLL_ACCESS_NAME = read_object', 'COLL_ACCESS_NAME = modify_object'])

            self.admin.assert_icommand(['iadmin', 'rmuser', test_user_name])
            self.admin.assert_icommand(['iadmin', 'rmgroup', test_group_name])

            # Perms should vanish after deletion of associated users
            self.admin.assert_icommand(['iquest', "SELECT COLL_ACCESS_USER_ID, COLL_ACCESS_NAME WHERE COLL_NAME = '%s' AND COLL_ACCESS_USER_ID %s" % (os.path.join(self.admin.session_collection, test_collection_name), query_string)], 'STDOUT_SINGLELINE', 'CAT_NO_ROWS_FOUND: Nothing was found matching your query')
        finally:
            self.admin.run_icommand(['iadmin', 'rmuser', test_user_name])
            self.admin.run_icommand(['iadmin', 'rmgroup', test_group_name])
