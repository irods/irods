...
namespace
{
                           ...
                           const dml::insert_op& _op) -> int
    {
        ...
        try {
            ...
            for (auto&& row : _op.rows) {
                nanodbc::prepare(stmt, sql.data());

                const auto& parent_coll_name = row.at("parent_coll_name");          // Cannot be null
                const auto& coll_name = row.at("coll_name");                        // Cannot be null
                const auto& coll_owner_name = row.at("coll_owner_name");            // Cannot be null
                const auto& coll_owner_zone = row.at("coll_owner_zone");            // Cannot be null
                const auto coll_map_id = std::stoi(row.value("coll_map_id", "0"));  // Cannot be null
                const auto coll_inheritance = row.value("coll_inheritance", "");
                const auto coll_type = row.value("coll_type", "");
                const auto coll_info_1 = row.value("coll_info1", "");
                const auto coll_info_2 = row.value("coll_info2", "");
                        ...
                        const dml::insert_op& _op) -> int
    {
        ...
        try {
            ...
            for (auto&& row : _op.rows) {
                ...
                const auto& coll_id = std::stoi(row.at("coll_id"));                           // Cannot be null
                const auto& data_name = row.at("data_name");                                 // Cannot be null
                const auto data_repl_num = std::stoi(row.at("data_repl_num"));               // Cannot be null
                const auto data_version = row.value("data_version", "0");
                const auto& data_type_name = row.at("data_type_name");                       // Cannot be null
                const auto data_size = row.value("data_size", "0");                          // Cannot be null
                const auto resc_group_name = row.value("resc_group_name", "EMPTY_RESC_GROUP_NAME");
                const auto resc_name = row.value("resc_name", "EMPTY_RESC_NAME");            // Cannot be null
                const auto& data_path = row.at("data_path");                                 // Cannot be null
                const auto& data_owner_name = row.at("data_owner_name");                     // Cannot be null
                const auto& data_owner_zone = row.at("data_owner_zone");                     // Cannot be null
                const auto data_is_dirty = std::stoi(row.value("data_is_dirty", "0"));
                const auto data_status = row.value("data_status", "");
                const auto data_checksum = row.value("data_checksum", "");
                const auto data_expiry_ts = row.value("data_expiry_ts", "");
                const auto data_map_id = std::stoll(row.value("data_map_id", "0"));
                const auto data_mode = row.value("data_mode", "");
                const auto r_comment = row.value("r_comment", "");
                const auto create_ts = row.value("create_ts", timestamp);
                const auto modify_ts = row.value("modify_ts", timestamp);
                const auto resc_hier = row.value("resc_hier", "EMPTY_RESC_HIER");
                const auto resc_id = std::stoi(row.at("resc_id"));                          // This is required.
                         ...
                         const dml::insert_op& _op) -> int
    {
        ...
        try {
            ...
            for (auto&& row : _op.rows) {
                ...
                const auto meta_namespace = row.value("meta_namespace", "");
                const auto& meta_attr_name = row.at("meta_attr_name");          // Cannot be null
                const auto& meta_attr_value = row.at("meta_attr_value");        // Cannot be null
                const auto meta_attr_unit = row.value("meta_attr_unit", "");
                const auto r_comment = row.value("r_comment", "");
                const auto create_ts = row.value("create_ts", timestamp);
                const auto modify_ts = row.value("modify_ts", timestamp);
                              ...
                              const dml::insert_op& _op) -> int
    {
        ...
        try {
            ...
            for (auto&& row : _op.rows) {
                ...
                const auto& object_id = row.at("object_id");  // Cannot be null
                const auto& meta_id = row.at("meta_id");      // Cannot be null
                const auto create_ts = row.value("create_ts", timestamp);
                const auto modify_ts = row.value("modify_ts", timestamp);
                         ...
                         const dml::insert_op& _op) -> int
    {
        ...
        try {
            ...
            for (auto&& row : _op.rows) {
                ...
                const auto& resc_name = row.at("resc_name");              // Cannot be null
                const auto& zone_name = row.at("zone_name");              // Cannot be null
                const auto& resc_type_name = row.at("resc_type_name");    // Cannot be null
                const auto& resc_class_name = row.at("resc_class_name");  // Cannot be null
                const auto& resc_net = row.at("resc_net");                // Cannot be null
                const auto& resc_def_path = row.at("resc_def_path");      // Cannot be null
                const auto free_space = row.value("free_space", "");
                const auto free_space_ts = row.value("free_space_ts", "");
                const auto resc_info = row.value("resc_info", "");
                const auto r_comment = row.value("r_comment", "");
                     ...
                     const dml::delete_op& _op) -> int
    {
        ...
        try {
            const auto& conditions = _op.conditions;
            ...
            // - One that holds the values for the prepared statement.
            std::for_each(std::begin(conditions), std::end(conditions),
                [&_op, &condition_clauses, &values](const dml::condition& _c)
                {
                    throw_if_invalid_table_column(_c.column, _op.table);
                    throw_if_invalid_conditional_operator(_c.op);

                    condition_clauses.push_back(fmt::format("{} {} ?", _c.column, _c.op));
                    values.push_back(_c.values.front());
                });
            ...
            // Generate the final SQL.
            const auto sql = fmt::format("delete from {} where {}", to_upper_copy(_op.table), conditions_stream.str());
            rodsLog(LOG_NOTICE, "%s :: Generated SQL = [%s]", __func__, sql.data());
                     ...
                     const dml::update_op& _op) -> int
    {
        ...
        try {
            for (auto&& update : _op.updates) {
                const auto& columns = update.row.columns;

                if (columns.empty()) {
                    return SYS_INVALID_INPUT_PARAM;
                }

                const auto& conditions = update.conditions;
                ...
                std::vector<std::string> set_clauses;
                set_clauses.reserve(columns.size());

                std::vector<std::string> bind_args;
                bind_args.reserve(columns.size() + conditions.size());

                // Build two lists of strings.
                // - One that holds the SQL set syntax.
                // - One that holds the values for the prepared statement.
                for (auto&& [name, value] : columns) {
                    throw_if_invalid_table_column(name, _op.table);
                    set_clauses.push_back(fmt::format("{} = ?", name));
                    bind_args.push_back(value);
                }
                ...
                // - One that holds the values for the prepared statement.
                std::for_each(std::begin(conditions), std::end(conditions),
                    [&_op, &condition_clauses, &bind_args](const dml::condition& _c)
                    {
                        throw_if_invalid_table_column(_c.column, _op.table);
                        throw_if_invalid_conditional_operator(_c.op);

                        condition_clauses.push_back(fmt::format("{} {} ?", _c.column, _c.op));
                        bind_args.push_back(_c.values.front());
                    });
                ...
                // Generate the final SQL.
                const auto sql = fmt::format("update {} set {} where {}", to_upper_copy(_op.table), set_stream.str(), conditions_stream.str());
                rodsLog(LOG_NOTICE, "%s :: Generated SQL = [%s]", __func__, sql.data());
    ...
    auto generic_insert(nanodbc::connection& _db_conn,
                        const std::string_view db_instance_name,
                        const dml::insert_op& _op) -> int
    {
        if (auto iter = handlers.find({"insert", _op.table}); iter == std::end(handlers)) {
            rodsLog(LOG_ERROR, "%s :: Invalid operation [(insert, %s)].", __func__, _op.table.data());
            return SYS_INVALID_INPUT_PARAM;
        }
    ...
    auto generic_update(nanodbc::connection& _db_conn,
                        const std::string_view db_instance_name,
                        const dml::update_op& _op) -> int
    {
        if (auto iter = handlers.find({"update", _op.table}); iter == std::end(handlers)) {
            rodsLog(LOG_ERROR, "%s :: Invalid operation [(update, %s)].", __func__, _op.table.data());
            return SYS_INVALID_INPUT_PARAM;
        }
    ...
    auto generic_delete(nanodbc::connection& _db_conn,
                        const std::string_view db_instance_name,
                        const dml::delete_op& _op) -> int
    {
        if (auto iter = handlers.find({"delete", _op.table}); iter == std::end(handlers)) {
            rodsLog(LOG_ERROR, "%s :: Invalid operation [(delete, %s)].", __func__, _op.table.data());
            return SYS_INVALID_INPUT_PARAM;
        }
    ...
    {
        auto t = _s;
        std::transform(std::begin(t), std::end(t), std::begin(t), [](unsigned char _c) { return std::toupper(_c); });
        return t;
    } // to_upper
...
namespace irods::experimental
{
    namespace dml
    {
        auto row::at(const std::string_view _column_name) const -> const std::string&
        {
            const auto end = std::end(columns);
            const auto iter = std::find_if(std::begin(columns), end, [&_column_name](const column& _v) {
        ...
        auto row::value(const std::string_view _column_name,
            ...
            try {
                for (auto&& op : _ops) {
                    const auto ec = std::visit(overloaded{
                        [&db_conn, &db_instance_name](const dml::insert_op& _op) -> int {
                            return generic_insert(db_conn, db_instance_name, _op);
                        },
                        [&db_conn, &db_instance_name](const dml::update_op& _op) -> int {
                            return generic_update(db_conn, db_instance_name, _op);
                        },
                        [&db_conn, &db_instance_name](const dml::delete_op& _op) -> int {
                            return generic_delete(db_conn, db_instance_name, _op);
                        }
                    }, op);
