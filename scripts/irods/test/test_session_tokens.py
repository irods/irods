...
class test_session_token_lifetime_configuration(unittest.TestCase):
	...
	def setUpClass(self):
		...
		self.test_user = session.mkuser_and_return_session("rodsuser", "bilbo", "baggins", lib.get_hostname())
		...
		self.test_user.environment_file_contents.update(client_update)
	...
	def tearDownClass(self):
		...
		with session.make_session_for_existing_admin() as admin_session:
			admin_session.assert_icommand(["iadmin", "rmuser", self.test_user.username])
...
class test_password_authentication_returning_session_tokens(unittest.TestCase):
	...
	def do_test_expiring_session_tokens_returned_when_password_is_correct(self,user):
		...
		user.assert_icommand(["ils"], "STDERR", "AUTHENTICATION_ERROR")
