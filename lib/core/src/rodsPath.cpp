...
int getLastPathElement(char* _logical_path, char* _last_path_element)
{
    ...
    if (std::string_view sv = _logical_path; fs::path::preferred_separator == sv.back()) {
        ...
        rstrcpy(_last_path_element, s.data(), MAX_NAME_LEN);
        return 0;
    }

    auto path = fs::path{_logical_path}.object_name();
    ...
    if (fs::path::preferred_separator == p.back()) {
        sv = sv.substr(0, sv.size() - 1);
    }

    rstrcpy(_last_path_element, sv.data(), MAX_NAME_LEN);
