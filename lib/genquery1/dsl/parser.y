                                   ...
                                   std::optional<std::string_view> _fn) -> void
{
    ...
    if (_fn) {
        // clang-format off
        if      (boost::iequals(*_fn, "order"))      { addInxIval(&_drv.gq_input->selectInp, column_id, ORDER_BY); }
        else if (boost::iequals(*_fn, "order_desc")) { addInxIval(&_drv.gq_input->selectInp, column_id, ORDER_BY_DESC); }
        else if (boost::iequals(*_fn, "min"))        { addInxIval(&_drv.gq_input->selectInp, column_id, SELECT_MIN); }
        else if (boost::iequals(*_fn, "max"))        { addInxIval(&_drv.gq_input->selectInp, column_id, SELECT_MAX); }
        else if (boost::iequals(*_fn, "sum"))        { addInxIval(&_drv.gq_input->selectInp, column_id, SELECT_SUM); }
