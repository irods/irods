...
auto list_logical_quotas(const char* _coll_name = nullptr) -> int
{
    ...
    if (status < 0) {
        ...
        const char* error_name = rodsErrorName(status, &sub_error_name);
        fmt::print(stderr,
                   "rc_get_logical_quota failed with error [{}] and suberror [{}] ec=[{}]",
                   error_name,
                   (sub_error_name ? sub_error_name : "N/A"),
                   status);
        printErrorStack(Conn->rError);
...
doCommand( char *cmdToken[], rodsArguments_t* _rodsArgs = 0 ) {
    ...
    if (strcmp(cmdToken[0], "set_logical_quota") == 0) {
        ...
        generalAdmin(0, cmdToken[0], cmdToken[1], cmdToken[2], cmdToken[3], "", "", "", "", "", "");
        return 0;
    }
...
usage( char *subOpt ) {
    ...
    char* moduserMsgs[] = {
        " moduser Name[#Zone] [ type | comment | info | password | remove_password ] newValue [no-scramble]",
        ...
        "to indicate that the password should not be scrambled before being sent to the server.",
        " ",
    ...
    char* modrescMsgs[] = {" modresc Name [name, type, host, path, status, comment, info, free_space, context, "
                           "parent_context, rebalance] Value",
                           " ",
    ...
    char* set_grid_configuration_usage[] = {"set_grid_configuration NAMESPACE OPTION_NAME OPTION_VALUE",
                                            " ",
                                            ...
                                            "access_time      queue_name_prefix",
                                            "access_time      queue_size",
    ...
    char* list_logical_quotas_usage[] = {"list_logical_quotas [COLLECTION_NAME]",
                                         " ",
    ...
    char* calculate_logical_usage_usage[] = {"calculate_logical_usage",
                                             " ",
