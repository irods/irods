...
usage() {
    char *msgs[] = {
        ...
        " ",
        "If 'uppercase' (or 'upper') appears before the query_string, the database value",
        "in the 'WHERE' condition will be made upper case so one can do case-insensitive",
        "tests (using upper-case literals).",
        " ",
        ...
        "  iquest uppercase \"select COLL_NAME, DATA_NAME WHERE DATA_NAME LIKE 'F1'\"  use uppercase values during WHERE for case-insensitive search",
