// IMPORTANT
// ~~~~~~~~~
// This test will fail if the iRODS server is running.
// The replica access table API uses unique identifiers in its implementation.
// If the server is running, then this unit-test's use of the API will fail
...
auto insert_new_entry(ix::replica_access_table& rat, access_info& info) -> void;
auto append_to_entry(ix::replica_access_table& rat, access_info& info, std::string token) -> void;
...
TEST_CASE("replica_access_table")
{
    ...
    irods::at_scope_exit cleanup{[] { ix::replica_access_table::deinit(); }};

    auto& rat = ix::replica_access_table::instance();

    // Insert a new entry into the table.
    REQUIRE_NOTHROW(insert_new_entry(rat, infos[0]));

    // Attempting to insert an entry that already exists generates an exception.
    REQUIRE_THROWS(insert_new_entry(rat, infos[0]), "replica_access_table: Entry already exists");

    // Because the next entry is referencing the same replica, it must use the token returned
    // by the first call to insert.
    REQUIRE_NOTHROW(append_to_entry(rat, infos[1], infos[0].token));

    // Insert a completely new entry because a different replica is being referenced.
    REQUIRE_NOTHROW(insert_new_entry(rat, infos[2]));
    ...
    SECTION("erase and restore entry with multiple PIDs")
    {
        ...
        auto [data_id, replica_number] = *entry_1;
        REQUIRE(rat.contains(data_id, replica_number));
        ...
        rat.restore(infos[0].token, data_id, replica_number, infos[0].pid);
        rat.restore(infos[1].token, data_id, replica_number, infos[1].pid);
        REQUIRE(rat.contains(data_id, replica_number));
        REQUIRE(rat.contains(infos[0].token, data_id, replica_number));
    }
