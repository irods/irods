...
auto rs_genquery2(RsComm* _comm, GenQuery2Input* _input, char** _output) -> int
{
    ...
    *_output = nullptr;
    ...
    if (const auto ec = getAndConnRcatHost(_comm, PRIMARY_RCAT, _input->zone, &host_info); ec < 0) {
        log_api::error("{}: Could not connect to remote zone [{}].", __func__, _input->zone);
        return ec;
    }
    ...
    try {
        // FIXME "gq" is pre-defined in the Bison grammar.
        // This needs to be removed or defined in a way that's internal/private to the grammar.
        gq::options opts;
        ...
        opts.admin_mode = irods::is_privileged_client(*_comm); // TODO Should probably be a choice for the admin.
        //opts.default_number_of_rows = 8; // TODO Can be pulled from R_GRID_CONFIGURATION on server startup.
