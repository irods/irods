...
#define NB_WRITE_TOUT_SEC       60      /* 60 sec timeout */
    ...
    eirods::error nonblocking_file_write_plugin( 
        ...
        while (toWrite > 0) {
            ...
            if (status == 0) {
                /* timedout */
                return ERROR( UNIX_FILE_OPR_TIMEOUT_ERR - errno, "nonblocking_file_write_plugin - time out error" );;
            } else if (status < 0) {
                if ( errno == EINTR) {
                    errno = 0;
                    continue;
                } else {
                    return ERROR( UNIX_FILE_WRITE_ERR - errno, "nonblocking_file_write_plugin - file write error" );
                }
            }
            #endif
    ...
    eirods::error nonblocking_file_unlink_plugin( 
        ...
        if( status < 0 ) {
            ...
            std::stringstream msg;
            msg << "nonblocking_file_unlink_plugin: unlink error for ";
            msg << fco.physical_path();
            msg << ", errno = '";
            msg << strerror( errno );
            msg << "', status = ";
    ...
    eirods::error nonblocking_file_stat_plugin( 
        ...
        //        prepended - hcj
        if( !_ctx ) {
            return ERROR( SYS_INVALID_INPUT_PARAM, "nonblocking_file_stat_plugin - invalid resource context" );
        }
        ...
        if( status < 0 ) {
            ...
            std::stringstream msg;
            msg << "nonblocking_file_stat_plugin: stat error for ";
            msg << fco.physical_path();
            msg << ", errno = '";
            msg << strerror( errno );
            msg << "', status = ";
    ...
    eirods::error nonblocking_file_fstat_plugin( 
        ...
        if( status < 0 ) {
            ...
            std::stringstream msg;
            msg << "nonblocking_file_fstat_plugin: fstat error for ";
    ...
    eirods::error nonblocking_file_lseek_plugin( 
        ...
        if( status < 0 ) {
            ...
            std::stringstream msg;
            msg << "nonblocking_file_lseek_plugin: lseek error for ";
            msg << fco.file_descriptor();
            msg << ", errno = '";
            msg << strerror( errno );
            msg << "', status = ";
    ...
    eirods::error nonblocking_file_fsync_plugin( 
        ...
        if( status < 0 ) {
            ...
            std::stringstream msg;
            msg << "nonblocking_file_fsync_plugin: fsync error for ";
            msg << fco.file_descriptor();
            msg << ", errno = '";
            msg << strerror( errno );
            msg << "', status = ";
    ...
    eirods::error nonblocking_file_chmod_plugin( 
        eirods::resource_operation_context* _ctx ) { 
        // =-=-=-=-=-=-=-
        // Check the operation parameters and update the physical path
        eirods::error ret = unix_check_params_and_path( _ctx );
        ...
        if( status < 0 ) {
            ...
            std::stringstream msg;
            msg << "nonblocking_file_chmod_plugin: chmod error for ";
            msg << fco.physical_path();
            msg << ", errno = '";
            msg << strerror( errno );
            msg << "', status = ";
    ...
    eirods::error nonblocking_file_mkdir_plugin( 
        ...
        if( !_ctx ) {
            return ERROR( SYS_INVALID_INPUT_PARAM, "nonblocking_file_mkdir_plugin - invalid resource context" );
        }
    ...
    // =-=-=-=-=-=-=-
    // interface for POSIX readdir
    eirods::error nonblocking_file_stage_plugin( 
        eirods::resource_operation_context* _ctx ) { 
        return CODE( SYS_NOT_SUPPORTED );

    } // nonblocking_file_stage_plugin
