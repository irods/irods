...
'''
    ...
    def test_msiExecCmd_closeAllL1Desc__issue_6623(self):
        ...
        pep_map = {
            'irods_rule_engine_plugin-irods_rule_language': textwrap.dedent(
                ...
                pep_resource_close_post(*inst, *ctx, *out) {{
                    ...
                    msiAddKeyVal(*key_val_pair,"{}","*time_str");
                    msiAssociateKeyValuePairsToObj(*key_val_pair,"{}","-d");
        ...
        try:
            with temporary_core_file() as core:
                ...
                with open(rule_file, 'w') as f:
                    f.write(rule_map[self.plugin_name].format(logical_path))
                ...
                expected_count = 1
                q = "select COUNT(META_DATA_ATTR_ID) where META_DATA_ATTR_NAME = '{}'".format(attr)
    ...
    def test_rsDataObjRepl_populates_input_struct__issue_6100(self):
        ...
        pep_map = {
            'irods_rule_engine_plugin-irods_rule_language': textwrap.dedent(
                '''pep_api_data_obj_repl_post(*inst, *comm, *inp, *stat) {{
                    msiGetValByKey(*inp, "resc_hier", *src);
                    msiAddKeyVal(*src_kvp,"{0}_src", "*src");
                    msiAssociateKeyValuePairsToObj(*src_kvp, "{1}", "-d");

                    msiGetValByKey(*inp, "dest_resc_hier", *dest);
                    msiAddKeyVal(*dest_kvp,"{0}_dest", "*dest");
                    msiAssociateKeyValuePairsToObj(*dest_kvp, "{1}", "-d");
                }}
            '''.format(metadata_attr, logical_path))
        }
        ...
        try:
            with temporary_core_file() as core:
                ...
                self.assertTrue(lib.metadata_attr_with_value_exists(self.admin, '{}_src'.format(metadata_attr), resc1))
                self.assertTrue(lib.metadata_attr_with_value_exists(self.admin, '{}_dest'.format(metadata_attr), resc2))
