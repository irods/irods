...
irods::error build_sorted_child_vector(
    ...
    size_t list_size = cmap_ref->size();
...
irods::error round_robin_start_operation(
    ...
    std::vector< std::string > child_vector;
    irods::error err = build_sorted_child_vector( _prop_map, child_vector );
    if ( !err.ok() ) {
...
irods::error round_robin_file_resolve_hierarchy(
    irods::plugin_context& _ctx,
    const std::string*               _opr,
    const std::string*               _curr_host,
    irods::hierarchy_parser*        _out_parser,
    ...
    if ( !err.ok() ) {
        return PASSMSG(
                   "round_robin_resolve_hierarchy - bad resource context", err );
    }
    if ( !_opr ) {
        return ERROR(
                   SYS_INVALID_INPUT_PARAM,
                   "round_robin_resolve_hierarchy - null operation" );
    }
    if ( !_curr_host ) {
        return ERROR(
                   SYS_INVALID_INPUT_PARAM,
                   "round_robin_resolve_hierarchy - null host" );
    }
    if ( !_out_parser ) {
        return ERROR(
                   SYS_INVALID_INPUT_PARAM,
                   "round_robin_resolve_hierarchy - null outgoing hier parser" );
    }
    if ( !_out_vote ) {
        return ERROR(
                   SYS_INVALID_INPUT_PARAM,
                   "round_robin_resolve_hierarchy - null outgoing vote" );
    }
    ...
    if ( irods::OPEN_OPERATION  == ( *_opr )  ||
        ...
        // =-=-=-=-=-=-=-
        // forward the resolve_hierarchy call to the child for assertion of the whole operation,
    ...
    std::stringstream msg;
    msg << "round_robin_resolve_hierarchy - operation not supported [";
    msg << ( *_opr ) << "]";
    return ERROR( -1, msg.str() );
