...
namespace
{
    // A support structure that is used to group audit information.
    // This type is meant to be used with the function, execute_sql.
    struct audit_info
    {
    ...
    irods::error execute_sql(const std::string_view _sql, const audit_info& _audit_info)
    {
        ...
        if (0 != ec) {
            rodsLog(LOG_ERROR, "SQL execution error [%i].", ec);
            return ERROR(ec, "SQL execution error.");
        }

        ec = cmlAudit3(AU_MOD_TICKET,
                       _audit_info.ticket_id.data(),
                       _audit_info.username.data(),
                       _audit_info.zone.data(),
                       _audit_info.tag.data(),
                       &icss);
        if (0 != ec) {
            return ERROR(ec, fmt::format("cmlAudit3 [{}] failed.", _audit_info.tag));
        }
                                                   ...
                                                   const KeyValPair* _cond_input)
    {
        ...
        if (std::strcmp(_op_name, "delete") == 0) {
            ...
            if (0 != ec && CAT_SUCCESS_BUT_WITH_NO_INFO != ec) {
                rodsLog(LOG_WARNING, "Failed to delete ticket information [error_code=%i, ticket=%s, table=R_TICKET_ALLOWED_HOSTS]",
                        ec, ticket_id_string.c_str());
            }
            ...
            if (0 != ec && CAT_SUCCESS_BUT_WITH_NO_INFO != ec) {
                rodsLog(LOG_WARNING, "Failed to delete ticket information [error_code=%i, ticket=%s, table=R_TICKET_ALLOWED_USERS]",
                        ec, ticket_id_string.c_str());
            }
            ...
            if (0 != ec && CAT_SUCCESS_BUT_WITH_NO_INFO != ec) {
                rodsLog(LOG_WARNING, "Failed to delete ticket information [error_code=%i, ticket=%s, table=R_TICKET_ALLOWED_GROUPS]",
                        ec, ticket_id_string.c_str());
            }

            ec = cmlAudit3(AU_DELETE_TICKET, ticket_id_string.c_str(),
                           _ctx.comm()->clientUser.userName,
                           _ctx.comm()->clientUser.rodsZone,
                           _ticket_string, &icss);
            if (0 != ec) {
                return ERROR(ec, "cmlAudit3 ticket string failure.");
            }
        ...
        audit_info.ticket_id = ticket_id_string;
        audit_info.username = _ctx.comm()->clientUser.userName;
        audit_info.zone = _ctx.comm()->clientUser.rodsZone;
        ...
        if (std::strcmp(_op_name, "mod") == 0) {
            if (std::strcmp(_arg3, "uses") == 0) {
                ...
                return execute_sql("update R_TICKET_MAIN set uses_limit = ? where ticket_id = ?", audit_info);
            } // uses
            ...
            if (std::strcmp(_arg3, "write-file") == 0) {
                ...
                audit_info.tag = "write file";

                return execute_sql("update R_TICKET_MAIN set write_file_limit = ? where ticket_id = ?", audit_info);
            } // write-file
            ...
            if (std::strcmp(_arg3, "write-bytes") == 0) {
                ...
                audit_info.tag = "write byte";

                return execute_sql("update R_TICKET_MAIN set write_byte_limit = ? where ticket_id = ?", audit_info);
            } // write-bytes
            ...
            if (std::strcmp(_arg3, "expire") == 0) {
                ...
                audit_info.tag = "expire";

                return execute_sql("update R_TICKET_MAIN set ticket_expiry_ts = ? where ticket_id = ?", audit_info);
            } // expire
            ...
            if (std::strcmp(_arg3, "add") == 0) {
                if (std::strcmp(_arg4, "host") == 0) {
                    ...
                    audit_info.tag = "add host";

                    return execute_sql("insert into R_TICKET_ALLOWED_HOSTS (ticket_id, host) values (? , ?)", audit_info);
                } // host
                ...
                if (std::strcmp(_arg4, "user") == 0) {
                    ...
                    audit_info.tag = "add user";

                    return execute_sql("insert into R_TICKET_ALLOWED_USERS (ticket_id, user_name) values (? , ?)", audit_info);
                } // user
                ...
                if (std::strcmp(_arg4, "group") == 0) {
                    ...
                    audit_info.tag = "add group";

                    return execute_sql("insert into R_TICKET_ALLOWED_GROUPS (ticket_id, group_name) values (? , ?)", audit_info);
                } // group
            } // add
                ...
                if (std::strcmp(_arg4, "host") == 0) {
                    ...
                    audit_info.tag = "remove host";

                    return execute_sql("delete from R_TICKET_ALLOWED_HOSTS where ticket_id = ? and host = ?", audit_info);
                } // host
                ...
                if (std::strcmp(_arg4, "user") == 0) {
                    ...
                    audit_info.tag = "remove user";

                    return execute_sql("delete from R_TICKET_ALLOWED_USERS where ticket_id = ? and user_name = ?", audit_info);
                } // user
                ...
                if (std::strcmp(_arg4, "group") == 0) {
                    ...
                    audit_info.tag = "remove group";

                    return execute_sql("delete from R_TICKET_ALLOWED_GROUPS where ticket_id = ? and group_name = ?", audit_info);
                } // group
            } // remove
        } // modify ticket operation
...
//        _ctx.prop_map().get< icatSessionStruct >( ICSS_PROP, icss );
    ...
    audit_info audit_info;
    audit_info.ticket_id = ticketIdStr;
    audit_info.username = _ctx.comm()->clientUser.userName;
    audit_info.zone = _ctx.comm()->clientUser.rodsZone;
    ...
    if ( strcmp( _op_name, "mod" ) == 0 ) {
        if (strcmp(_arg3, "uses") == 0) {
            ...
            return execute_sql("update R_TICKET_MAIN set uses_limit=? where ticket_id = ? and user_id = ?", audit_info);
        } // uses
        ...
        if ( strcmp(_arg3, "write-file") == 0 ) {
            ...
            return execute_sql("update R_TICKET_MAIN set write_file_limit=? where ticket_id = ? and user_id = ?", audit_info);
        } // write-file
        ...
        if (strcmp(_arg3, "write-bytes") == 0) {
            ...
            return execute_sql("update R_TICKET_MAIN set write_byte_limit=? where ticket_id = ? and user_id = ?", audit_info);
        } // write-bytes
        ...
        if (strcmp(_arg3, "expire") == 0 ) {
            ...
            return execute_sql("update R_TICKET_MAIN set ticket_expiry_ts=? where ticket_id = ? and user_id = ?", audit_info);
        } // expire
...
irods::database* plugin_factory(
    ...
    pg->add_operation<userInfo_t*>(
        ...
        function<error(plugin_context&,userInfo_t*)>(
            db_reg_user_re_op ) );
    pg->add_operation<const char*,const char*,const char*,const char*,const char*,const KeyValPair*>(
    ...
    pg->add_operation<const char*,const char*,const char*,const char*,const char*,const KeyValPair*>(
        ...
        function<error(plugin_context&,const char*,const char*,const char*,const char*,const char*,const KeyValPair*)>(
            db_add_avu_metadata_op ) );
    pg->add_operation<const char*,const char*,const char*,const char*,const char*,const char*,const char*,const char*,const KeyValPair*>(
        ...
        function<error(plugin_context&,const char*,const char*,const char*,const char*,const char*,const char*,const char*,const char*,const KeyValPair*)>(
            db_mod_avu_metadata_op ) );
    pg->add_operation<int,const char*,const char*,const char*,const char*,const char*,int,const KeyValPair*>(
        ...
        function<error(plugin_context&,int,const char*,const char*,const char*,const char*,const char*,int,const KeyValPair*)>(
            db_del_avu_metadata_op ) );
    pg->add_operation<const char*,const char*,const char*,const char*,const KeyValPair*>(
