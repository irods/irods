...
class Test_Resource_Unixfilesystem(ResourceSuite, ChunkyDevTest, unittest.TestCase):
    ...
    def test_unix_filesystem_free_inodes_for_create__3195(self):
        if statvfs_path_or_parent(lib.get_vault_path(self.admin, 'demoResc')).f_files == 0:
...
class Test_Resource_Random(ChunkyDevTest, ResourceSuite, unittest.TestCase):
    ...
    def test_unix_filesystem_high_water_mark__2981(self):
        ...
        lib.mkdir_p(lib.get_vault_path(self.admin, 'unix1Resc'))
        lib.mkdir_p(lib.get_vault_path(self.admin, 'unix2Resc'))
        lib.mkdir_p(lib.get_vault_path(self.admin, 'unix3Resc'))
...
class Test_Resource_RoundRobin(ChunkyDevTest, ResourceSuite, unittest.TestCase):
    ...
    @unittest.skipIf(configuration.RUN_IN_TOPOLOGY, "local filesystem check")
    def test_unix_filesystem_free_inodes_for_create__3195(self):
        if statvfs_path_or_parent(lib.get_vault_path(self.admin, 'unix1Resc')).f_files == 0:
            return # file system doesn't report inodes
        if statvfs_path_or_parent(lib.get_vault_path(self.admin, 'unix2Resc')).f_files == 0:
            return # file system doesn't report inodes
