...
rodsLog( LOG_NOTICE, "XXXX - chkCollForBundleOpr :: replicate data [%s], coll [%s], resc hier [%s]", curCollEnt->dataName, curCollEnt->collName, resc_hier );
			    status = replDataObjForBundle( rsComm, curCollEnt->collName, curCollEnt->dataName, 
                                               //resource, 0, NULL);
                                               resc_hier, 0, NULL);
			if (status < 0) {
                            rodsLog (LOG_ERROR,
