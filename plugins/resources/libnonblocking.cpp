...
#define NB_WRITE_TOUT_SEC	60	/* 60 sec timeout */
    ...
    // =-=-=-=-=-=-=-
	// interface for non-blocking Read
	eirods::error nbFileReadPlugin( eirods::resource_property_map* 
	                                                      _prop_map, 
						            eirods::resource_child_map* 
									                      _cmap,
                                    eirods::first_class_object* 
		...
		char *tmpPtr;
		fd_set set;
        int fd = _object->file_descriptor();
		...
		while (toRead > 0) {
			...
			nbytes = read( _object->file_descriptor(), (void *) tmpPtr, toRead );
			if (nbytes < 0) {
				if (errno == EINTR) {
					/* interrupted */
					errno = 0;
    ...
    // =-=-=-=-=-=-=-
	// interface for non-blocking Write
	eirods::error nbFileWritePlugin( eirods::resource_property_map* 
	                                                       _prop_map, 
									   eirods::resource_child_map*
									                       _cmap,
                                      eirods::first_class_object* 
		...
		while (toWrite > 0) {
			...
			nbytes = write ( _object->file_descriptor(), (void *) tmpPtr, _len );
			if (nbytes < 0) {
				if (errno == EINTR) {
					/* interrupted */
					errno = 0;
	...
	eirods::resource* plugin_factory( std::string _context  ) {
        ...
        resc->add_operation( "read",         "nbFileReadPlugin" );
        resc->add_operation( "write",        "nbFileWritePlugin" );
