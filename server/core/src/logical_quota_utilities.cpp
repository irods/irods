...
namespace irods::logical_quotas
{
    int check_logical_quota_violation(RsComm* _comm, const char* _coll_name)
    {
        ...
        if (LOCAL_HOST == rodsServerHost->localFlag) {
            ...
            try {
                const auto client_user_type =
                    *ua::server::type(*_comm, ua::user{_comm->clientUser.userName, _comm->clientUser.rodsZone});
                if (ua::user_type::rodsadmin != client_user_type) {
                    // Allow a privilege escalation to fetch the grid config value.
                    auto local_admin =
                        irods::experimental::fully_qualified_username{_comm->myEnv.rodsUserName, _comm->myEnv.rodsZone};
                    auto conn = irods::experimental::client_connection{
                        rodsServerHost->hostName->name, _comm->myEnv.rodsPort, local_admin};
                ...
                log_server::error("{}: Caught iRODS exception with ec=[{}] while connecting to catalog provider. "
                                  "Logical quotas will not be enforced. Exception: [{}]",
                                  __func__,
                                  e.code(),
                ...
                log_server::error("{}: Caught std::exception while connecting to catalog provider. Logical quotas will "
                                  "not be enforced. Exception: [{}]",
                                  __func__,
                                  e.what());
                return static_cast<int>(violation::none);
            }
            catch (...) {
                log_server::error("{}: Caught unknown error while connecting to catalog provider. Logical quotas will "
                                  "not be enforced.",
                                  __func__);
                return static_cast<int>(violation::none);
            }
        ...
        if (status < 0) {
            log_server::warn(
                "{}: Failed to get logical quota enforcement status. (ec=[{}]) Logical quotas will not be enforced.",
                __func__,
                status);
            return static_cast<int>(violation::none);
        }
        ...
        if (std::strncmp(gcout->option_value, "1", 2) != 0) {
            ...
            log_server::warn("{}: Received unknown value [{}] when fetching logical quota enforcement status. (Set to "
                             "\"1\" to enable enforcement.) Logical quotas will not be enforced.",
                             __func__,
                             gcout->option_value);
            return static_cast<int>(violation::none);
        }
        ...
        for (int i = 0; i < out->len && ((violation_flags ^ violation::bytes ^ violation::objects) != violation::none);
             i++)
        {
