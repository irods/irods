...
class Test_IQuery(
    session.make_sessions_mixin(rodsadmins, rodsusers), unittest.TestCase
):
    ...
    def test_iquery_can_run_a_query__issue_7570(self):
        ...
        self.user.assert_icommand(
            [
                "iquery",
                f"select COLL_NAME where COLL_NAME = '{self.user.session_collection}'",
            ],
            "STDOUT",
            [json_string],
        )
    ...
    def test_iquery_supports_reading_query_from_stdin__issue_7570(self):
        ...
        self.user.assert_icommand(
            ...
            "STDOUT",
            [json_string],
            input=f"select COLL_NAME where COLL_NAME = '{self.user.session_collection}'",
        )

    def test_iquery_returns_error_on_invalid_zone__issue_7570(self):
        ec, out, err = self.user.assert_icommand_fail(
            ["iquery", "-z", "invalid_zone", "select COLL_NAME"], "STDOUT"
        )
        ...
        ec, out, err = self.user.assert_icommand_fail(
            ["iquery", "-z", "", "select COLL_NAME"], "STDOUT"
        )
        self.assertEqual(ec, 1)
        self.assertEqual(len(out), 0)
        self.assertEqual(err, "error: -26000\n")  # SYS_INVALID_ZONE_NAME
    ...
    def test_iquery_returns_error_on_invalid_query_tokens__issue_7570(self):
        ...
        ec, out, err = self.user.assert_icommand_fail(
            ["iquery", "select INVALID_COLUMN"], "STDOUT"
        )
        self.assertEqual(ec, 1)
        self.assertEqual(len(out), 0)
        self.assertEqual(err, "error: -130000\n")  # SYS_INVALID_INPUT_PARAM

    def test_iquery_returns_error_when_closing_single_quote_is_missing__issue_6393(
        self,
    ):
        ec, out, err = self.user.assert_icommand_fail(
            [
    ...
    def test_iquery_distinguishes_embedded_IN_substring_from_IN_operator__issue_3064(
        self,
    ):
        ...
        try:
            ...
            for op in ["IN", "in"]:
                with self.subTest(f"op={op}"):
                    ...
                    self.user.assert_icommand(
                        ["iquery", query_string], "STDOUT", [json_string]
                    )
    ...
    def test_iquery_supports_OR_operator__issue_4069(self):
        ...
        json_string = json.dumps(
            [[self.user.home_collection], [self.admin.home_collection]],
            separators=(",", ":"),
        )
    ...
    def test_iquery_supports_embedded_single_quotes__issue_5727(self):
        ...
        escaped_strings = [
            ...
            data_object.replace(
                "'", "''"
            ),  # Uses two single quotes to escape single quote like in SQL.
        ]
        json_string = json.dumps(
            [[data_object, self.user.session_collection]], separators=(",", ":")
        )
    ...
    def test_iquery_returns_error_on_invalid_query_string__issue_5734(self):
        ...
        # in the WHERE clause.
        query_string = (
            "select DATA_ID, DATA_REPL_NUM, COLL_NAME, DATA_NAME, DATA_RESC_HIER, DATA_PATH, META_DATA_ATTR_VALUE "
        ...
        ec, out, err = self.user.assert_icommand_fail(
            ["iquery", query_string], "STDOUT"
        )
        self.assertEqual(ec, 1)
        self.assertEqual(len(out), 0)
        self.assertEqual(err, "error: -167000\n")  # SYS_LIBRARY_ERROR

    def test_genquery2_maps_genquery_user_zone_columns_to_correct_database_columns__issue_8134_8135(
        self,
    ):
        ...
        for line in out.splitlines():
            if "(R_USER_MAIN.zone_name)" in line:
                if "COLL_ACCESS_USER_ZONE" in line:
                    collection_mapping_is_correct = True
                elif "DATA_ACCESS_USER_ZONE" in line:
                    data_object_mapping_is_correct = True
        ...
        json_string = json.dumps(
            [[self.user.username, self.user.zone_name, "own"]], separators=(",", ":")
        )
        ...
        json_string = json.dumps(
            [[self.user.username, self.user.zone_name, "1"]], separators=(",", ":")
        )
        self.user.assert_icommand(["iquery", query_string], "STDOUT", [json_string])
    ...
    def test_iquery_correctly_parses_queries_with_tabs_in_the_where_clause__issue_7795(
        self,
    ):
        # Run a basic query to get the expected output.
        query_string = f"select USER_ID where USER_NAME = '{self.user.username}'"
        expected_output = self.user.assert_icommand(["iquery", query_string], "STDOUT")[
            1
        ].strip()
        ...
        query_string_with_tab_in_where_clause = (
            query_string + "\tand USER_TYPE = 'rodsuser'"
        )

        # iquest will succeed because it uses a real parser now.
        user_id = self.user.assert_icommand(
            ["iquest", "%s", query_string_with_tab_in_where_clause], "STDOUT"
        )[1].strip()
        ...
        output = self.user.assert_icommand(
            ["iquery", query_string_with_tab_in_where_clause], "STDOUT"
        )[1].strip()
            ...
            with self.subTest(f"user type is [{user_type}]"):
                try:
                    self.admin.assert_icommand(
                        ["iadmin", "moduser", self.user.username, "type", user_type]
                    )
                    self.admin.assert_icommand(
                        ["iuserinfo", self.user.username],
                        "STDOUT",
                        ["type: " + user_type],
                    )
                    ...
                    self.admin.assert_icommand(
                        ["iadmin", "atg", group, self.user.username]
                    )
                    ...
                    self.user.assert_icommand(
                        [
                            "iquery",
                            f"select DATA_NAME where DATA_NAME = '{data_object}'",
                        ],
                        "STDOUT",
                        ["[]"],
                    )
                    ...
                    self.admin.assert_icommand(
                        ["ichmod", "-M", "read", group, self.user.session_collection]
                    )
                    json_string = json.dumps([[self.user.session_collection]])
                    self.user.assert_icommand(
                        [
                    ...
                    self.admin.assert_icommand(
                        [
                            ...
                            "read",
                            group,
                            f"{self.user.session_collection}/{data_object}",
                        ]
                    )
                    ...
                    json_string = json.dumps(
                        [[self.user.session_collection, data_object]],
                        separators=(",", ":"),
                    )
                    self.user.assert_icommand(
                        ["iquery", query_string], "STDOUT", [json_string]
                    ...
                    self.admin.run_icommand(
                        ["iadmin", "moduser", self.user.username, "type", "rodsuser"]
                    )

    def test_genquery2_sql_generation_for_sql_distinct_keyword__issue_8261(self):
        with self.subTest(
            "GenQuery2 does not automatically insert DISTINCT keyword into SQL"
        ):
            _, out, _ = self.user.assert_icommand(
                ["iquery", "--sql-only", "select DATA_NAME"], "STDOUT"
            )
            sql = out.strip()
        ...
        with self.subTest("GenQuery2 supports SELECT DISTINCT syntax"):
            _, out, _ = self.user.assert_icommand(
                ["iquery", "--sql-only", "select distinct DATA_NAME"], "STDOUT"
            )
        ...
        with self.subTest("GenQuery2 supports DISTINCT keyword in aggregate functions"):
            _, out, _ = self.user.assert_icommand(
                ["iquery", "--sql-only", "select count(DATA_NAME)"], "STDOUT"
            )
            self.assertTrue(out.strip().startswith("select count(t0.data_name)"))

            _, out, _ = self.user.assert_icommand(
                ["iquery", "--sql-only", "select count(distinct DATA_NAME)"], "STDOUT"
            )
            self.assertTrue(
                out.strip().startswith("select count(distinct t0.data_name)")
            )
            ...
            self.user.assert_icommand(
                [
                    "iquery",
                    "--sql-only",
                    "select count(DATA_NAME) order by count(DATA_NAME)",
                ],
                "STDOUT",
                [" order by count(t0.data_name) "],
            )
            ...
            self.user.assert_icommand(
                [
                    "iquery",
                    "--sql-only",
                    "select count(distinct DATA_NAME) order by count(distinct DATA_NAME)",
                ],
                "STDOUT",
                [" order by count(distinct t0.data_name) "],
            )
            ...
            _, out, _ = self.user.assert_icommand(
                ["iquery", "--sql-only", "select issue_8261_fn(DATA_NAME)"], "STDOUT"
            )
            self.assertTrue(
                out.strip().startswith("select issue_8261_fn(t0.data_name)")
            )

            _, out, _ = self.user.assert_icommand(
                ["iquery", "--sql-only", "select issue_8261_fn(distinct DATA_NAME)"],
                "STDOUT",
            )
            self.assertTrue(
                out.strip().startswith("select issue_8261_fn(distinct t0.data_name)")
            )
            ...
            self.user.assert_icommand(
                [
                    "iquery",
                    "--sql-only",
                    "select issue_8261_fn(distinct DATA_NAME) order by issue_8261_fn(distinct DATA_NAME)",
                ],
                "STDOUT",
                [" order by issue_8261_fn(distinct t0.data_name) "],
            )
    ...
    def test_genquery2_handling_of_distinct_keyword__issue_8261(self):
        ...
        try:
            ...
            with self.subTest("List logical name of replicas"):
                ...
                json_string = json.dumps(
                    [[data_object], [data_object], [data_object]], separators=(",", ":")
                )
                self.user.assert_icommand(
                    ["iquery", query_string], "STDOUT", [json_string]
                )
    ...
    @unittest.skipUnless(
        test.settings.TOPOLOGY_FROM_RESOURCE_SERVER,
        "Test must be run from a Catalog Service Consumer",
    )
    def test_genquery2_does_not_fail_when_log_level_is_set_to_trace_and_request_is_redirected_to_the_provider__issue_8439(
        self,
    ):
        config = IrodsConfig()
        ...
        try:
            with lib.file_backed_up(config.server_config_path):
                ...
                lib.update_json_file_from_dict(
                    config.server_config_path, config.server_config
                )
                controller.reload_configuration()
                self.user.assert_icommand(
                    ["iquery", "select COLL_NAME"],
                    "STDOUT",
                    [json.dumps([self.user.session_collection])],
                )

        finally:
            controller.reload_configuration()

    def test_iquery_correctly_parses_queries_with_functions_in_the_group_by_clause(
        self,
    ):
        # Run a basic query to get the expected output.
