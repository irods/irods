...
namespace
{
    ...
    int _rsDataObjRename(
        ...
        if( irods::KW_CFG_SERVICE_ROLE_CONSUMER == svc_role ) {
            ...
            log_api::error("{}: role not supported [{}]", __FUNCTION__, svc_role.c_str());
            return SYS_SERVICE_ROLE_NOT_SUPPORTED;
        }

        if (!rsComm) {
            log_api::error("{}: rsComm is null", __FUNCTION__);
        ...
        if ( ( status = splitPathByKey(
            ...
            log_api::error(
                "{}: splitPathByKey for {} error, status = {}", __FUNCTION__, srcDataObjInp->objPath, status);
            return status;
        }
        if ( ( status = splitPathByKey(
        ...
        if ( srcDataObjInp->oprType == RENAME_DATA_OBJ ) {
            ...
            if ( status < 0 ) {
                log_api::error(
                    "{}: src coll {} does not exist, status = {}", __FUNCTION__, srcDataObjInp->objPath, status);
                return status;
            }
        }
        else {
            if ( ( status = isData( rsComm, srcDataObjInp->objPath, &srcId ) ) >= 0 ) {
                ...
                log_api::error(
                    "{}: src obj {} does not exist, status = {}", __FUNCTION__, srcDataObjInp->objPath, status);
                return status;
            }
        }
        ...
        if (srcDataObjInp->oprType == RENAME_DATA_OBJ &&
            ...
            log_api::error("{}: cannot rename tar bundle type obj {}", __FUNCTION__, srcDataObjInp->objPath);
        ...
        if ( strcmp( srcObj, destObj ) != 0 ) {
            ...
            if ( i < 0 ) {
                ...
                log_api::error(
                    "{}: acPreProcForObjRename error for source {} and destination {},stat={}",
                    __FUNCTION__,
                    args[0],
                    args[1],
                    i);
        ...
        if ( strcmp( srcColl, destColl ) != 0 ) {
            ...
            if (status < 0) {
                log_api::error(
                    "{}: Destination collection does not exist [collection = {}, error_code = {}]",
                    __FUNCTION__,
                    destColl,
                    status);
            ...
            if ( acPreProcFromRenameFlag == 0 ) {
                ...
                if ( i < 0 ) {
                    ...
                    log_api::error(
                        "rsDataObjRename: acPreProcForObjRename error for source {} and destination {},stat={}",
                        args[0],
                        args[1],
                        i);
                    return i;
        ...
        if ( status >= 0 ) {
            ...
            if ( status >= 0 ) {
                ...
                if ( status < 0 ) {
                    ...
                    log_api::error(
                        "rsDataObjRename: acPostProc err for src {} dest {},stat={}", args[0], args[1], status);
                }
            }
        }
        else {
    ...
    int rsDataObjRename_impl(rsComm_t *rsComm, dataObjCopyInp_t *dataObjRenameInp)
    {
        ...
        if ( destType >= 0 ) {
            log_api::error("rsDataObjRename: dest specColl objPath {} exists", destDataObjInp->objPath);
            freeDataObjInfo( srcDataObjInfo );
        ...
        if ( srcType >= 0 ) {
            ...
            if ( NULL != srcDataObjInfo && SYS_SPEC_COLL_OBJ_NOT_EXIST == destType ) {
                ...
                log_api::error(
                    "rsDataObjRename: src {} is in spec coll but dest {} is not",
                    srcDataObjInp->objPath,
                    destDataObjInp->objPath);
            ...
            log_api::error(
                "rsDataObjRename: src {} is not in spec coll but dest {} is",
                srcDataObjInp->objPath,
                destDataObjInp->objPath);
...
int rsDataObjRename(rsComm_t *rsComm, dataObjCopyInp_t *dataObjRenameInp)
{
    ...
    // Allow if source and destination paths are the same.
    if (src_path.parent_path() != dest_path.parent_path()) {
        // Since some fs operations may be slightly heavier, guard
        ...
        try {
            if ((violation_flags & lq::violation::objects) != lq::violation::none) {
                ...
                quota_violated = !fs::server::is_data_object_registered(*rsComm, dest_path);
            }

            if (!quota_violated && ((violation_flags & lq::violation::bytes) != lq::violation::none)) {
                const std::uintmax_t src_size = fs::server::is_data_object_registered(*rsComm, src_path)
                                                    ? fs::server::data_object_size(*rsComm, src_path)
                                                    : 0;
                ...
                const std::uintmax_t dest_size = (((violation_flags & lq::violation::objects) != lq::violation::none) ||
                                                  fs::server::is_data_object_registered(*rsComm, dest_path))
                                                     ? fs::server::data_object_size(*rsComm, dest_path)
                                                     : 0;
        ...
        // consider things like size or existence.
            ...
            log_api::error(
                "{}: Caught iRODS filesystem error with ec=[{}] while processing logical quotas. Logical quotas will "
                "be enforced without consideration of source/destination object properties. Exception: [{}]",
                __func__,
                e.code().value(),
                e.what());
            ...
            log_api::error(
                "{}: Caught iRODS exception with ec=[{}] while processing logical quotas. Logical quotas will be "
                "enforced without consideration of source/destination object properties. Exception: [{}]",
                __func__,
                e.code(),
            ...
            log_api::error("{}: Caught std::exception while processing logical quotas. Logical quotas will be enforced "
                           "without consideration of source/destination object properties. Exception: [{}]",
                           __func__,
                           e.what());
            quota_violated = (violation_flags != lq::violation::none);
        }
        catch (...) {
            log_api::error("{}: Caught unknown error while processing logical quotas. Logical quotas will be enforced "
                           "without consideration of source/destination object properties.",
                           __func__);
            quota_violated = (violation_flags != lq::violation::none);
        }
    }
    ...
    if (quota_violated) {
        log_api::info("{}: Logical quota violation on collection [{}] with source [{}] and destination [{}].",
                      __func__,
...
specCollObjRename( rsComm_t *rsComm, dataObjInfo_t *srcDataObjInfo,
    ...
    if ( status < 0 ) {
        log_api::error(
            "specCollObjRename: l3Rename error from {} to {}, status = {}", srcDataObjInfo->subPath, newPath, status);
        return status;
    }
    return status;
}
...
moveMountedCollDataObj( rsComm_t *rsComm, dataObjInfo_t *srcDataObjInfo,
    ...
    if ( status < 0 ) {
        log_api::error(
            "moveMountedCollDataObj: getFilePathName err for {}. status = {}", destDataObjInfo.objPath, status);
        return status;
    }
    ...
    if ( status == LOCAL_DIR_T ) {
        ...
        log_api::error("moveMountedCollDataObj: targ path {} is a dir. status = {}", destDataObjInfo.filePath, status);
        return status;
    }
        ...
        if ( status == 1 ) {
            ...
            if (error_code < 0) {
                log_api::error(
                    "renameFilePathToNewDir failed in moveMountedCollDataObj with error code {}", error_code);
            }
    ...
    if ( status < 0 ) {
        log_api::error(
            "moveMountedCollDataObj: l3Rename error from {} to {}, status = {}",
            srcDataObjInfo->filePath,
    ...
    if ( status < 0 ) {
        ...
        log_api::error(
            "moveMountedCollDataObj: rsRegDataObj for {} failed, status = {}", destDataObjInfo.objPath, status);
    }
    return status;
}
...
moveMountedCollCollObj( rsComm_t *rsComm, dataObjInfo_t *srcDataObjInfo,
    ...
    while ( rsFileReaddir( rsComm, &fileReaddirInp, &rodsDirent ) >= 0 ) {
        ...
        if ( status < 0 || fileStatOut == NULL ) { // JMC cppcheck
            log_api::error(
                "moveMountedCollCollObj: l3Stat for {} error, status = {}", subSrcDataObjInfo.filePath, status);
            return status;
        }
        ...
        if ( status < 0 ) {
            savedStatus = status;
            log_api::error(
                "moveMountedCollCollObj: moveMountedColl for {} error, stat = {}", subSrcDataObjInfo.objPath, status);
        }
        free( fileStatOut );
        fileStatOut = NULL;
    }
