...
///
/// This function assumes that the coll_name inside of the structure is also heap-allocated. Do not use this function if
/// that pointer is not to be free()'d.
///
...
///
/// \parblock
/// On success, *_logicalQuotaList will be a pointer to a heap-allocated LogicalQuotaList.
/// Within *_logicalQuotaList, there will be a pointer to a heap-allocated array of "len" LogicalQuota structs.
/// This array must be free()'d by the caller to avoid leaks. *_logicalQuotaList must also be free()'d to avoid leaks.
/// To avoid free()'ing the LogicalQuotaList internals by hand, clear_logical_quota_list() can be used.
...
/// \endparblock
/// \param[out] _logicalQuotaList A pointer to a LogicalQuotaList pointer that will hold the results.
///
...
///
/// \since 5.1.0
int rc_get_logical_quota(struct RcComm* _conn,
                         getLogicalQuotaInp_t* _getLogicalQuotaInp,
