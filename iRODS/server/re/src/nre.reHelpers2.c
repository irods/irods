...
// precond: char array escaped_string must has size MAX_COND_LEN
int
insertEscape(char *escaped_string, char *val) {
    int len = strlen(val);
    int i=0;
    int j=0; // index in the escaped string
    while(i<len) {
        if(j>=MAX_COND_LEN-2) { // we need two extra chars one for possible \ one for \0
        ...
        switch(val[i]) {
            ...
            case '\'':
            case '\\':
                // insert extra backslash
                escaped_string[j++] = '\\';
                escaped_string[j++] = val[i++];
                break;
            default:
                // copy
                escaped_string[j++] = val[i++];
...
{
    char escaped_string[MAX_COND_LEN];
    if(insertEscape(escaped_string, val) == SYS_COPY_LEN_ERR)
        return SYS_COPY_LEN_ERR;
  int vl,sl;
  
  vl = strlen(escaped_string);
  sl = strlen(start);

  if (rmemmove((char*)(start+ vl),(char *) (start+paramLen),
	       sl-paramLen+1,size - vl) == NULL)
    return(SYS_COPY_LEN_ERR);
  if (rmemmove(start,escaped_string,vl,vl) == NULL)
    return(SYS_COPY_LEN_ERR);
  return(0);
}
int
