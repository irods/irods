...
usage() {
    const char *msgs[] = {
        ...
        "specified. Files and directories prepended with 'i:' are iRODS data objects and",
        ...
        "and target locations to determine whether synchronization is needed. Therefore,",
        "the command will run faster if the checksum value for the specific iRODS data object,",
        "no matter whether it is a source or target, already exists and is registered",
        "in the catalog. This can be achieved by using the -k or -K options of the iput",
        "command at the time of ingestion, or by using the ichksum command after the",
        "data have already been ingested into iRODS.",

        "If the -s option is used, only the file size (instead of the size and",
        "checksum value) is used for determining whether synchronization is needed.",
        "This mode provides a faster operation but the result provides less assurance.",
        " ",
        "The command accepts multiple sourceFiles|sourceDirectories and a single",
        "targetFile|targetDirectory. It follows the syntax of the UNIX",
        "cp command with one exception - irsync of a single source directory to a ",
        "single target collection. In UNIX, the command:",
        " ",
        "     cp -r foo1 foo2",
        " ",
        "has a different meaning depending on whether the target directory foo2",
        "already exists. If the target directory foo2 already exists, the content",
        "of source directory foo1 is copied into the target directory as foo2/foo1.",
        "But if the target directory foo2 does not already exist, the content is",
        "copied as the target directory foo2.",
        " ",
        ...
        "foo2, regardless of whether foo2 already exists.",
        " ",
