...
inline irods::error load_balanced_check_params(irods::plugin_context& _ctx)
{
...
///        to pass on the call
template<typename DEST_TYPE>
irods::error load_balanced_get_resc_for_call(irods::plugin_context& _ctx,
                                             irods::resource_ptr&   _resc)
{
    // check incoming parameters
    if (const auto err = load_balanced_check_params<DEST_TYPE>(_ctx); !err.ok()) {
...
irods::error load_balanced_file_create(irods::plugin_context& _ctx)
{
    ...
    if (const auto err = load_balanced_get_resc_for_call<irods::file_object>(_ctx, resc); !err.ok()) {
...
irods::error load_balanced_file_read(irods::plugin_context& _ctx,
                                     void*                  _buf,
                                     const int              _len)
{
    // get the child resc to call
    irods::resource_ptr resc;
    if (const auto err = load_balanced_get_resc_for_call<irods::file_object>(_ctx, resc); !err.ok()) {
        return PASSMSG("Failed finding resource.", err);
    }
...
irods::error load_balanced_file_write(irods::plugin_context& _ctx,
                                      const void*            _buf,
                                      const int              _len)
{
    // get the child resc to call
    irods::resource_ptr resc;
    if (const auto err = load_balanced_get_resc_for_call<irods::file_object>(_ctx, resc); !err.ok()) {
        return PASSMSG("Failed choosing child resource.", err);
    }
...
irods::error load_balanced_file_close(irods::plugin_context& _ctx)
{
    ...
    irods::resource_ptr resc;
    if (const auto err = load_balanced_get_resc_for_call<irods::file_object>(_ctx, resc); !err.ok()) {
        return PASSMSG("Failed to select load_balanced resource.", err);
    }
...
irods::error load_balanced_file_unlink(irods::plugin_context& _ctx)
{
    // get the child resc to call
    irods::resource_ptr resc;
    if (const auto err = load_balanced_get_resc_for_call<irods::data_object>(_ctx, resc); !err.ok()) {
        return PASSMSG("Failed to select load_balanced resource.", err);
    }
...
irods::error load_balanced_file_stat(irods::plugin_context& _ctx,
                                     struct stat*           _statbuf)
{
    // get the child resc to call
    irods::resource_ptr resc;
    if (const auto err = load_balanced_get_resc_for_call<irods::data_object>(_ctx, resc); !err.ok()) {
        return PASSMSG("Failed selecting load_balanced child resource.", err);
    }
...
irods::error load_balanced_file_lseek(irods::plugin_context& _ctx,
                                      ...
                                      const int              _whence)
{
    ...
    irods::resource_ptr resc;
    if (const auto err = load_balanced_get_resc_for_call<irods::file_object>(_ctx, resc); !err.ok()) {
        return PASSMSG("Failed to select load_balanced child.", err);
    }
...
irods::error load_balanced_file_mkdir(irods::plugin_context& _ctx)
{
    // get the child resc to call
    irods::resource_ptr resc;
    if (const auto err = load_balanced_get_resc_for_call<irods::collection_object>(_ctx, resc); !err.ok()) {
        return PASSMSG("Failed to select load_balanced child resource.", err);
    }
...
irods::error load_balanced_file_readdir(irods::plugin_context& _ctx,
...
irods::error load_balanced_file_rename(irods::plugin_context& _ctx,
...
irods::error load_balanced_file_stage_to_cache(irods::plugin_context& _ctx,
                                               const char*            _cache_file_name)
{
    ...
    // call stage to cache on the child
...
///        is not used.
irods::error load_balanced_file_sync_to_arch(irods::plugin_context& _ctx,
                                             const char*            _cache_file_name)
{
    // get the child resc to call
    irods::resource_ptr resc;
    ...
    // call sync to archive on the child
...
irods::error load_balanced_file_registered(irods::plugin_context& _ctx)
{
    ...
    irods::resource_ptr resc;
    if (const auto err = load_balanced_get_resc_for_call<irods::file_object>(_ctx, resc); !err.ok()) {
        return PASSMSG("Failed selecting load_balanced resource.", err);
    }
...
irods::error load_balanced_file_notify(irods::plugin_context& _ctx,
...
irods::error load_balanced_file_resolve_hierarchy(
    ...
    // =-=-=-=-=-=-=-
    // check incoming parameters
    irods::error ret = load_balanced_check_params< irods::file_object >( _ctx );
    if ( !ret.ok() ) {
    ...
    if ( irods::OPEN_OPERATION   == ( *_opr )  ||
        ...
        ret = load_balanced_redirect_for_open_operation( _ctx, _opr, _curr_host, _out_parser, _out_vote );
        if ( !ret.ok() ) {
