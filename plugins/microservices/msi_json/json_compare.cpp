...
namespace
{
                  ...
                  ruleExecInfo_t* _rei) -> int
    {
        ...
        try {
            ...
            rodsLog(LOG_ERROR, "Caught exception while comparing JSON objects. [%s]", e.client_display_what());
            return static_cast<int>(e.code());
        }
        catch (const boost::bad_any_cast& e) {
            rodsLog(LOG_ERROR, "Caught exception while comparing JSON objects. [%s]", e.what());
