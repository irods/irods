...
// TODO show that R_GRID_CONFIGURATION is honored
...
auto get_replica_info(RcComm& _comm, const fs::path& _logical_path, int _replica_number)
    -> std::tuple<std::string, std::string, std::string>
{
    const auto query_string = fmt::format("select DATA_ID, DATA_ACCESS_TIME, DATA_MODIFY_TIME where COLL_NAME = '{}' and DATA_NAME = '{}'",
    ...
    throw std::logic_error{
        fmt::format("Precondition violation: replica does not exist: logical_path=[{}], replica_number=[{}]",
            _logical_path.c_str(), _replica_number)};
} // get_replica_info
