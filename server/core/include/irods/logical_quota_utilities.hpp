...
namespace irods::logical_quotas
{
    ...
    ///
    /// \warning operator<< and operator>> are intentionally not defined. As the underlying type is signed,
    /// these should stay undefined, as calling operator << on a negative value may cause undefined behavior.
    ///
    /// \since 5.1.0
    ...
    inline T operator_impl(
        T _lhs,
        T _rhs,
        Op<typename std::underlying_type_t<T>> op = Op<typename std::underlying_type_t<T>>()) noexcept
    {
        return static_cast<T>(op(static_cast<typename std::underlying_type_t<T>>(_lhs),
    ...
    ///
    /// This function calls rs_get_logical_quota and interprets its results.
    ...
    /// \retval <0 on failure.
    /// \retval >=0 integer valued such that the the bitwise AND (&) of the return value and a
    /// irods::logical_quotas::violation will be nonzero if the corresponding limit was violated.
    /// (i.e. (return value & irods::logical_quotas::violation::bytes) != 0 indicates byte limit was violated)
    ///
    /// \since 5.1.0
