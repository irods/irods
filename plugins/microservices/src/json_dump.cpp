...
namespace
{
    ...
    auto msi_impl(MsParam* _json_handle, MsParam* _json_pointer, MsParam* _string, ruleExecInfo_t* _rei) -> int
    {
        ...
        try {
            ...
            log_msi::error("Caught exception while serializing the JSON structure. [{}]", e.client_display_what());
            return static_cast<int>(e.code());
        }
        catch (const boost::bad_any_cast& e) {
            log_msi::error("Caught exception while serializing the JSON structure. [{}]", e.what());
