...
private:
	// for metadata dialog 
	int m_metadataDlgIdCounter;
	CStringArray m_objIdArrayForMetadataDlg;
	std::vector<int> m_dlgIdArray;
	std::vector<CMetadataDlg *> m_metadataDlgPointArray;

	CMetaSearchDlg *m_metaSearchDlg;

	HICON m_hIcon;

public:
	void CreateMetadataDlg(CString & ObjNameWpath, int ObjType);
	void CreateMetaSeachDlg(CString & TopColl);
	void StartProgressBar();
	void EndProgressBar();
	void SetProgressBarPos(float t);
	void ProgressBarStepIt();
	void ProgressBarMove2Full();
...
public:
	...
	void FillResourceList(CStringArray & RescNames, CString & default_resc_from_login);
	...
	LRESULT onMetadataDlgClosed(WPARAM wParam, LPARAM lParam);
	void statusbar_msg(CString & msg);
	void statusbar_msg2(CString & msg);
	void statusbar_clear();
	CString get_default_resc();
...
protected:
	...
	afx_msg void OnBnClickedStorageResource();
	//}}AFX_MSG
	DECLARE_MESSAGE_MAP()
};
