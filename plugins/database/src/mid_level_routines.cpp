...
cmlGetNextSeqVal( icatSessionStruct *icss ) {
    ...
    if ( status < 0 ) {
        log_db::info("cmlGetNextSeqVal cmlGetIntegerValueFromSql failure %d", status);
        return status;
    }
    return iVal;
}
...
cmlGetCurrentSeqVal( icatSessionStruct *icss ) {
    ...
    status = cmlGetIntegerValueFromSql( sql, &iVal, emptyBindVars, icss );
    if ( status < 0 ) {
        log_db::info("cmlGetCurrentSeqVal cmlGetIntegerValueFromSql failure %d", status);
        return status;
    }
    return iVal;
}
...
cmlGetNextSeqStr( char *seqStr, int maxSeqStrLen, icatSessionStruct *icss ) {
    ...
    if ( status < 0 ) {
        log_db::info("cmlGetNextSeqStr cmlGetStringValueFromSql failure %d", status);
    }
    return status;
}
