...
static int _delColl( rsComm_t *rsComm, collInfo_t *collInfo ) {
    ...
    // Remove associated logical quotas
    cllBindVars[cllBindVarCount++] = collIdNum;
    const auto ec = cmlExecuteNoAnswerSql("delete from R_LOGICAL_QUOTA_MAIN where coll_id=?", &icss);
    if (ec != 0 && ec != CAT_SUCCESS_BUT_WITH_NO_INFO) {
        log_db::warn("[{}:{}] - failed to delete associated logical quotas for [{}] [ec=[{}]]",
                     __func__,
                     __LINE__,
                     collInfo->collName,
                     ec);
                                                         ...
                                                         int _offset)
{
    ...
    if (_child_index >= _bundles->size()) {
        return ERROR(SYS_INVALID_INPUT_PARAM, "child index is greater than the number of bundles");
    }
    ...
    // Get all data ids not in the selected leaf resource, marked by _child_index
    std::string not_child_array;
    for (size_t idx = 0; idx < _bundles->size(); ++idx) {
        if (idx == _child_index || (*_bundles)[idx].empty()) {
            continue;
        }
    ...
    // Run query to collect data ids that may need to be
    // replicated to the selected leaf resource
#ifdef ORA_ICAT
                                     ...
                                     const char* _object_limit)
{
    ...
    {
        auto msg = fmt::format(
            "Invalid argument for logical quota byte limit. Could not convert [{}] to an integer.", byte_limit);
        log_db::error("{}: {}", __func__, msg);
        return ERROR(SYS_INVALID_INPUT_PARAM, std::move(msg));
    }

    if (const auto [ptr, ec] = std::from_chars(_object_limit, _object_limit + std::strlen(_object_limit), object_limit);
        ec != std::errc{})
    {
        auto msg = fmt::format(
            "Invalid argument for logical quota object limit. Could not convert [{}] to an integer.", object_limit);
        log_db::error("{}: {}", __func__, msg);
        return ERROR(SYS_INVALID_INPUT_PARAM, std::move(msg));
    }
...
irods::database* plugin_factory(
    ...
    pg->add_operation(DATABASE_OP_CALC_LOGICAL_USAGE_AND_QUOTA,
                      function<error(plugin_context&, const char*)>(db_calc_logical_usage_and_quota_op));
    pg->add_operation(DATABASE_OP_SET_LOGICAL_QUOTA,
