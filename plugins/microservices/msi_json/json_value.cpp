    ...
    auto msi_impl(MsParam* _json_handle, MsParam* _json_pointer, MsParam* _value, ruleExecInfo_t* _rei) -> int
    {
        ...
        try {
            ...
            if (json_value.is_string()) {
                ...
                rodsLog(LOG_DEBUG, "JSON value = [%i]", v);
                ...
                rodsLog(LOG_DEBUG, "JSON value = [null]");
                fillStrInMsParam(_value, "null");
            }
            else if (json_value.is_array() || json_value.is_object()) {
            ...
            rodsLog(LOG_ERROR, "Caught exception while fetching JSON value. [%s]", e.client_display_what());
            return static_cast<int>(e.code());
        }
        catch (const boost::bad_any_cast& e) {
            rodsLog(LOG_ERROR, "Caught exception while fetching JSON value. [%s]", e.what());
...
/// - null
///
/// Binary data is not supported by this microservice.
/// \endparblock
