...
namespace
{
    ...
    {
        // This function adds table JOINs for enforcing permissions. The addition of a table JOIN depends
        // on the GenQuery2 columns included in the query.
        //
        // Below is an example showing SQL that could be generated by GenQuery2. INNER JOINs are used when
        // the user is not a rodsadmin.
        //
        //     select ...
        //     from R_DATA_MAIN d
        //     inner join R_OBJT_ACCESS pdoa on pdoa.object_id = d.data_id
        ...
        //
        // However, when the user is a rodsadmin, LEFT JOINs are used instead. This allows rodsadmin to
        // find data objects and collections having no user permissions on them. Using INNER JOINs would
        // hide these objects otherwise.
        //
        // NOTE: It turns out that using LEFT JOINs appears to work for non-rodsadmins too. Extensive
        // testing is needed to be sure, but the idea is that non-rodsadmin queries always include a
        // permission check. For those tables which do not contain a match, a NULL value will be returned.
        // The NULL values will fail the permission check resulting in the same results as an INNER JOIN.
        // This idea depends on how the database treats NULL values during filtering.
        //
        // Columns in the ORDER-BY and GROUP-BY clauses do not affect the table JOINs.

        // R_OBJT_ACCESS holds all permissions for collections and data objects. This means that querying
        // information about permissions requires the joining of multiple tables. To be specific, the
        // following tables must be joined to gain access to permission information:
        //
        //     - R_OBJT_ACCESS  (holds permission associations between users, groups, and objects)
        //     - R_TOKN_MAIN    (holds symbolic names for permissions)
        //     - R_USER_GROUP   (holds group membership information, used for membership expansion)
        //     - R_USER_MAIN    (holds user/group information - e.g. name, type, etc.)
        //
        // All of those tables (except R_TOKN_MAIN) must be joined whenever permissions for a data object
        // are requested. Those same table must be joined again whenever permissions for a collection are
        // requested. For that reason, GenQuery2 predefines aliases for each table.
        //
        ...
        //     pdu          R_USER_MAIN
        //
        // Aliases starting with "pc" are for collection permissions. Aliases starting with "pd" are
        // for data object permissions.
        //
        // The aliases make it easier to manage query generation involving permissions.
        ...
        if (const auto iter = _state.table_aliases.find("R_COLL_MAIN"); iter != std::end(_state.table_aliases)) {
            // The following columns require the joining of R_OBJT_ACCESS:
            // - COLL_ACCESS_PERM_ID = R_OBJT_ACCESS.access_type_id
            // - COLL_ACCESS_USER_ID = R_OBJT_ACCESS.user_id
            [[maybe_unused]] auto added_r_objt_access_table = false;
            auto add_join =
            ...
            if (add_join) {
                added_r_objt_access_table = true;
                sql += fmt::format(
                    " {} join R_OBJT_ACCESS pcoa on {}.coll_id = pcoa.object_id", join_keyword, iter->second);
            }

            // The following columns require the joining of R_TOKN_MAIN:
            // - COLL_ACCESS_PERM_NAME = R_TOKN_MAIN.token_name
            //
            // The joining of R_TOKN_MAIN requires the following tables be joined:
            // - R_OBJT_ACCESS
            ...
            // The following columns require the joining of R_USER_MAIN:
            ...
            //
            // The joining of R_USER_MAIN requires the following tables be joined:
            // - R_OBJT_ACCESS
            // - R_USER_GROUP
            add_join =
                std::any_of(std::begin(_state.ast_column_ptrs), std::end(_state.ast_column_ptrs), [](auto* _col_ptr) {
    ...
    auto generate_condition_clause(gq_state& _state, const gq::options& _opts, const std::string& _conditions)
        -> std::string
    {
        // This function determines and adds the conditions for permissions based on the user's type.
        // If the user is a rodsadmin, no condition checks are added to the SQL (unless the user provided
        // their own permission-related checks).
        //
        // Enforcing permissions is accomplished through an existence check, as shown below.
        //
        //     where exists (
        //         select 1
        ...
        //         where oa.object_id = <R_COLL_MAIN_alias>.coll_id and
        //               u.user_name = ? and
        //               u.zone_name = ? and
        //               u.user_type_name != 'rodsgroup' and
        //               oa.access_type_id >= 1050
        //     )
        ...
        std::string sql;
        std::string condition_clause_prefix;
        ...
        if (c_iter != end) {
            sql += fmt::format(" {} exists ("
                               ...
                               " left join R_USER_GROUP ug on oa.user_id = ug.group_user_id"
                               " left join R_USER_MAIN u on ug.user_id = u.user_id "
                               "where oa.object_id = {}.coll_id"
                               " and u.user_name = ?"
                               " and u.zone_name = '{}'"
                               " and u.user_type_name != 'rodsgroup'"
                               " and oa.access_type_id >= 1050)",
                               condition_clause_prefix,
                               c_iter->second,
                               _opts.user_zone);
            _state.values.emplace_back(_opts.user_name);

            // Update the condition prefix in case we need to add the permission checks for
            // data objects.
            condition_clause_prefix = "and";
        }
