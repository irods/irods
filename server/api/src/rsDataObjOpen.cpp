...
int rsDataObjOpen(rsComm_t *rsComm, dataObjInp_t *dataObjInp)
{
    ...
    if (const auto access_mode = (dataObjInp->openFlags & O_ACCMODE); access_mode == O_WRONLY || access_mode == O_RDWR)
    {
    ...
    if (lq::violation::none != check_quota_flags) {
        ...
        if ((check_quota_flags & static_cast<lq::violation>(status)) != lq::violation::none) {
            log_api::info("{}: Logical quota violation on collection [{}] with status [{}]. Attempted to open [{}] "
                          "with openFlags [{:o}]",
                          __func__,
