    ...
    namespace logical_quotas {
        int check_logical_quota_violation(struct RsComm *_comm, const char* _coll_name) {
            ...
            if(status < 0) {
                ...
                return static_cast<int>(logical_quota_violation::NONE);
            }
            ...
            logical_quota_violation violation_flags = logical_quota_violation::NONE;
            for(int i = 0; i < out->len && ((violation_flags ^ logical_quota_violation::BYTES ^ logical_quota_violation::OBJECTS) != logical_quota_violation::NONE); i++) {
               if(out->list[i].over_bytes > 0) {
                    violation_flags |= logical_quota_violation::BYTES;
               }
