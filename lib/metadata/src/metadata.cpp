...
namespace irods::experimental::metadata
{
    ...
    {
        return _lhs.attribute == _rhs.attribute && _lhs.value == _rhs.value && _lhs.units == _rhs.units;
    }
    ...
    namespace
    {
        ...
        const std::map<entity_type, std::string> type_to_string{{entity_type::collection, "collection"},
                                                                {entity_type::data_object, "data_object"},
                                                                {entity_type::user, "user"},
                                                                {entity_type::resource, "resource"}};

        const std::map<entity_type, std::string> type_to_token{{entity_type::collection, tokens::collection},
                                                               {entity_type::data_object, tokens::data_object},
                                                               {entity_type::user, tokens::user},
                                                               {entity_type::resource, tokens::resource}};

        const std::map<std::string, entity_type> token_to_type{{tokens::collection, entity_type::collection},
                                                               {tokens::data_object, entity_type::data_object},
                                                               {tokens::user, entity_type::user},
                                                               {tokens::resource, entity_type::resource}};

        int mod_avu_meta(rxComm_t& _comm,
                         const avu& _md,
                         const avu& _dmd,
                         entity_type _et,
        ...
        {
            modAVUMetadataInp_t inp{const_cast<char*>(_op.c_str()),
        ...
        void throw_if_invalid(const M_T& _m, const T_T& _t)
        {
    ...
    {
        avu tmp{std::string{"n:"} + _dmd.attribute, std::string{"v:"} + _dmd.value, std::string{"u:"} + _dmd.units};
    ...
    auto get(rxComm_t& _comm, entity_type _et, const std::string& _tgt) -> std::vector<avu>
    {
        ...
        const std::map<entity_type, std::string> type_to_string{{entity_type::collection, "COLL"},
        ...
        try {
            ...
            THROW(SYS_UNKNOWN_ERROR, "Encountered an unexpected error getting AVU");
        }
