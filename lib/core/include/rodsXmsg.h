...
#ifndef RODS_XMSG_H__
#define RODS_XMSG_H__
...
#define SINGLE_MSG_TICKET  0	// ticket for just a single msg. ticket will be
                                // deleted after the msg is received
...
typedef struct IrodsXmsg {
    ...
    struct IrodsXmsg *tnext;     // the link list within the same ticket (diff msgNumber)
    struct IrodsXmsg *tprev;
    uint seqNumber;
    void *ticketMsgStruct;		 // points to the ticketMsgStruct_t this xmsg belongs
} irodsXmsg_t;
...
typedef struct TicketMsgStruct {
    ...
    void *ticketHashQue;           // points to the ticketHashQue_t this ticket belongs
    uint nxtSeqNumber;
} ticketMsgStruct_t;
