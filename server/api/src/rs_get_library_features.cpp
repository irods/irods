...
#define IRODS_FEATURE(F)        {#F, IRODS_FEATURE_VALUE(F)}, // NOLINT(cppcoreguidelines-macro-usage)
    ...
    *_features = strdup(nlohmann::json{
        ...
        IRODS_FEATURE(IRODS_LIBRARY_FEATURE_ACCESS_TIME)
        IRODS_FEATURE(IRODS_LIBRARY_FEATURE_AUTHENTICATE_CLIENT)
        IRODS_FEATURE(IRODS_LIBRARY_FEATURE_OVERRIDE_PACKING_INSTRUCTIONS)
        IRODS_FEATURE(IRODS_LIBRARY_FEATURE_CHECKSUM_ALGORITHM_CRC64NVME)
        IRODS_FEATURE(IRODS_LIBRARY_FEATURE_RESOURCES_READ_CHECKSUM_FROM_STORAGE_DEVICE)
        ...
        IRODS_FEATURE(IRODS_LIBRARY_FEATURE_RESOURCE_ADMINISTRATION)
    }.dump().c_str());
    // clang-format on
