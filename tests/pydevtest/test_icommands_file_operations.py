...
acSetRescSchemeForCreate { msiSetDefaultResc("demoResc","null"); }
    ...
    def test_ichksum_replica_reporting__3499(self):
        ...
        vault_session_path = lib.get_vault_session_path(self.admin)
        final_file_contents = 'b'
        with open(os.path.join(vault_session_path, filename), 'wb') as f:
            f.write(final_file_contents)
        rc, out, err = self.admin.run_icommand(['ichksum', '-KarR', 'demoResc', self.admin.session_collection])
    ...
    def test_ichksum_admin_flag__3265(self):
        ...
        _,data_path,_ = self.user0.run_icommand("ipwd")
