...
namespace
{
    ...
    int strToXmlStr(const char* inStr, char*& outXmlStr, const std::optional<irods::version>& peer_version);
        ...
        if (!_version.empty()) {
            ...
            if (std::smatch m; std::regex_match(_version, m, pattern)) {
                ...
                if (m.size() == 4) {
                    ...
                    rodsLog(LOG_NOTICE, "Could not extract major, minor, and patch information from string [_version=%s].",
                            _version.data());
                }
            }
        }
    ...
    auto use_correct_xml_encoding(const std::optional<irods::version>& _peer_version) noexcept -> bool
    {
        return _peer_version && *_peer_version >= irods::version{4, 2, 9};
    } // use_correct_xml_encoding
    ...
    initPackedOutput( const int len ) {
        return {
            .bBuf={
                .len=0,   // The amount of memory used in the buffer.
                .buf=malloc(len)
            },
            .bufSize=len, // Capacity
            .nopackBufArray={}
        };
    }
                           ...
                           const std::optional<irods::version>& peer_version)
    {
        ...
        case PACK_PI_STR_TYPE: {
            ...
            for ( i = 0; i < numStr; i++ ) {
                status = packString(inPtr, packedOutput, maxStrLen, myPackedItem.name, irodsProt, peer_version);
                if ( status < 0 ) {
                    rodsLog( LOG_ERROR,
        ...
        case PACK_STRUCT_TYPE:
            ...
            status = packChildStruct( inPtr, packedOutput, myPackedItem,
                                      myPackTable, numElement, packFlag, irodsProt,
                                      nullptr, peer_version );
                        ...
                        const std::optional<irods::version>& peer_version)
    {
        ...
        case PACK_PI_STR_TYPE: {
            ...
            for ( j = 0; j < numPointer; j++ ) {
                ...
                for ( i = 0; i < numStr; i++ ) {
                    status = packString(pointer, packedOutput, maxStrLen, myPackedItem.name, irodsProt, peer_version);
                    if ( status < 0 ) {
                        rodsLog( LOG_ERROR, "packPointerItem: strlen of %s > dim size, content: %s ", myPackedItem.name, pointer );
                        return status;
                    }
                }
                if ( ( packFlag & FREE_POINTER ) &&
    ...
    extendPackedOutput( packedOutput_t &packedOutput, int extLen, void *&outPtr ) {
        ...
        if ( newBufSize <= newOutLen || packedOutput.bufSize > MAX_PACKED_OUT_ALLOC_SZ ) {
            newBufSize = newOutLen + PACKED_OUT_ALLOC_SZ;
        }

        if (auto* tmp = realloc( packedOutput.bBuf.buf, newBufSize ); tmp) {
            packedOutput.bBuf.buf = tmp;
            packedOutput.bufSize = newBufSize;
        }
        else {
            packedOutput.bBuf.buf = nullptr;
        }

        if (!packedOutput.bBuf.buf) {
            rodsLog( LOG_ERROR, "extendPackedOutput: error malloc of size %d", newBufSize );
            outPtr = nullptr;
    ...
    int packItem(packItem_t& myPackedItem,
                 ...
                 const std::optional<irods::version>& peer_version)
    {
        if (const int ec = resolvePackedItem( myPackedItem, inPtr, PACK_OPR ); ec < 0) {
            return ec;
        }

        if ( myPackedItem.pointerType > 0 ) {
            /* a pointer type */
            return packPointerItem( myPackedItem, packedOutput, myPackTable, packFlag, irodsProt, peer_version);
        }

        return packNonpointerItem( myPackedItem, inPtr, packedOutput, myPackTable, packFlag, irodsProt, peer_version );
    }
    ...
    {
        if (irodsProt == XML_PROT) {
            return packXmlString(inPtr, packedOutput, maxStrLen, name, peer_version);
        }

        return packNatString(inPtr, packedOutput, maxStrLen);
    }
                      ...
                      const std::optional<irods::version>& peer_version)
    {
        ...
        char *xmlStr = nullptr;
        int xmlLen = strToXmlStr(static_cast<const char*>(inPtr), xmlStr, peer_version);
    ...
    int strToXmlStr(const char *inStr, char *&outXmlStr, const std::optional<irods::version>& peer_version)
    {
        ...
        for ( i = 0; inStr[i] != '\0'; ++i ) {
            switch(inStr[i]) {
                ...
                case '\'':
                    if (use_correct_xml_encoding(peer_version)) {
                        xml.write(inStr + copy_from, i - copy_from);
                        copy_from = i + 1;
                        xml << "&apos;";
                    }
    ...
    int xmlStrToStr(const char *inStr, int len, char *&outStr, const std::optional<irods::version>& peer_version)
    {
        outStr = nullptr;
        if ( inStr == NULL || len <= 0 ) {
            return 0;
                        ...
                        const std::optional<irods::version>& peer_version)
    {
        ...
        if ( packInstructInp == NULL ) {
            rodsLog( LOG_ERROR, "packChildStruct: matchPackInstruct failed for %s", myPackedItem.name );
...
#endif
                int status = packItem( *tmpItem, inPtr, packedOutput, myPackTable, packFlag, irodsProt, peer_version);
                if ( status < 0 ) {
                    return status;
                }
    ...
    int unpackItem(packItem_t& myPackedItem,
                   ...
                   const std::optional<irods::version>& peer_version)
    {
        if (const int ec = resolvePackedItem( myPackedItem, inPtr, UNPACK_OPR ); ec < 0) {
            return ec;
        }

        if ( myPackedItem.pointerType > 0 ) {
            /* a pointer type */
            return unpackPointerItem( myPackedItem, inPtr, unpackedOutput, myPackTable, irodsProt, peer_version);
        }

        return unpackNonpointerItem( myPackedItem, inPtr, unpackedOutput, myPackTable, irodsProt, peer_version);
    }
                             ...
                             const std::optional<irods::version>& peer_version)
    {
        ...
        case PACK_PI_STR_TYPE: {
            ...
            for ( i = 0; i < numStr; i++ ) {
                ...
                status = unpackString( inPtr, unpackedOutput, maxStrLen,
                                       myPackedItem.name, irodsProt, outStr, peer_version );
                if ( status < 0 ) {
                    rodsLog( LOG_ERROR,
        ...
        case PACK_STRUCT_TYPE:
            status = unpackChildStruct( inPtr, unpackedOutput, myPackedItem,
                                        myPackTable, numElement, irodsProt, nullptr, peer_version );
    ...
    {
        if (irodsProt == XML_PROT) {
            return unpackXmlString( inPtr, unpackedOutput, maxStrLen, name, outStr, peer_version);
        }

        return unpackNatString( inPtr, unpackedOutput, maxStrLen, outStr);
    }
                        ...
                        const std::optional<irods::version>& peer_version)
    {
        ...
        int extLen = maxStrLen;
        char* strBuf;
        myStrlen = xmlStrToStr((const char*) inPtr, origStrLen, strBuf, peer_version);
    ...
    {
        if (irodsProt == XML_PROT) {
            return unpackXmlStringToOutPtr(inPtr, outPtr, maxStrLen, name, peer_version);
        }
        
        return unpackNatStringToOutPtr(inPtr, outPtr, maxStrLen);
    }
                         ...
                         irodsProt_t irodsProt)
    {
        ...
        if ( irodsProt == XML_PROT ) {
            /* check if tag exists */
            int skipLen = 0;
            int tagLen = parseXmlTag( inPtr, myPackedItem.name, START_TAG_FL, skipLen );
            if ( tagLen < 0 ) {
                addPointerToPackedOut( unpackedOutput, 0, NULL );
                return 0;
            }
        ...
        else if ( strcmp( ( const char* )inPtr, NULL_PTR_PACK_STR ) == 0 ) {
            int myStrlen = strlen( NULL_PTR_PACK_STR );
            addPointerToPackedOut( unpackedOutput, 0, NULL );
            inPtr = ( const char * )inPtr + ( myStrlen + 1 );
            return 0;
                    ...
                    irodsProt_t irodsProt)
    {
        ...
        extendPackedOutput( unpackedOutput, sizeof( short ) * ( numElement + 1 ), outPtr );

        intValue = unpackInt16ToOutPtr( inPtr, outPtr, numElement, name, irodsProt );
                          ...
                          const std::optional<irods::version>& peer_version)
    {
        ...
        // If the caller did not provide any packing instructions via "packInstructInp",
        // then find the correct packing instruction by name (i.e. myPackedItem.name).
        if (!packInstructInp) {
            packInstructInp = matchPackInstruct( myPackedItem.name, myPackTable );
        }

        if ( packInstructInp == NULL ) {
            rodsLog( LOG_ERROR, "unpackChildStruct: matchPackInstruct failed for %s", myPackedItem.name );
            return SYS_UNMATCH_PACK_INSTRUCTI_NAME;
        }
        ...
        for (int i = 0; i < numElement; i++) {
            ...
            unpackItemHead.parent = &myPackedItem; // Link it.

            if ( irodsProt == XML_PROT ) {
                int skipLen = 0;
                int status = parseXmlTag( inPtr, myPackedItem.name, START_TAG_FL | LF_FL, skipLen );
...
#endif
                int status = unpackItem( *tmpItem, inPtr, unpackedOutput, myPackTable, irodsProt, peer_version );
                if ( status < 0 ) {
                    return status;
                }
            ...
            if ( irodsProt == XML_PROT ) {
                int skipLen = 0;
                int status = parseXmlTag( inPtr, myPackedItem.name, END_TAG_FL | LF_FL, skipLen );
                if ( status >= 0 ) {
                    inPtr = ( const char * )inPtr + status + skipLen;
                }
                else {
                          ...
                          const std::optional<irods::version>& peer_version)
    {
        ...
        if ( unpackNullString( inPtr, unpackedOutput, myPackedItem, irodsProt ) <= 0 ) {
        ...
        if ( myDim > 0 ) {
            if ( numPointer > 0 ) {
                ...
                if ( myTypeNum == PACK_DOUBLE_TYPE || myTypeNum == PACK_INT_TYPE || myTypeNum == PACK_INT16_TYPE ) {
        ...
        case PACK_BIN_TYPE:
            if ( myDim == 0 ) {
                if ( myPackedItem.pointerType != NO_PACK_POINTER ) {
                    outPtr = addPointerToPackedOut( unpackedOutput, numElement * elementSz, NULL );
                    status = unpackCharToOutPtr( inPtr, outPtr, numElement * elementSz, myPackedItem.name, myPackedItem.typeInx, irodsProt );
                }
        ...
        case PACK_PI_STR_TYPE: {
            ...
            if ( myDim == 0 ) {
                ...
                for ( i = 0; i < numStr; i++ ) {
                    status = unpackStringToOutPtr( inPtr, outPtr, maxStrLen, myPackedItem.name, irodsProt, peer_version );
                    if ( status < 0 ) {
                        return status;
                    }

                    if ( myTypeNum == PACK_PI_STR_TYPE && i == 0 && myOutStr != NULL ) {
        ...
        case PACK_INT_TYPE:
            if ( myDim == 0 ) {
                outPtr = addPointerToPackedOut( unpackedOutput, numElement * elementSz, NULL );
                unpackIntToOutPtr( inPtr, outPtr, numElement, myPackedItem.name, irodsProt );
                ...
                for ( i = 0; i < numPointer; i++ ) {
                    outPtr = pointerArray[i] = malloc( numElement * elementSz );
                    status = unpackIntToOutPtr( inPtr, outPtr, numElement * elementSz, myPackedItem.name, irodsProt );
                    if ( status < 0 ) {
                        return status;
                    }
                }
        ...
        case PACK_INT16_TYPE:
            if ( myDim == 0 ) {
                outPtr = addPointerToPackedOut( unpackedOutput, numElement * elementSz, NULL );
                unpackInt16ToOutPtr( inPtr, outPtr, numElement, myPackedItem.name, irodsProt );
                /* don't chk status. It could be a -ive int */
            }
            else {
                /* pointer to an array of pointers */
                for ( i = 0; i < numPointer; i++ ) {
                    outPtr = pointerArray[i] = malloc( numElement * elementSz );
                    status = unpackInt16ToOutPtr( inPtr, outPtr, numElement * elementSz, myPackedItem.name, irodsProt );
                    if ( status < 0 ) {
                        return status;
                    }
                }
        ...
        case PACK_DOUBLE_TYPE:
            if ( myDim == 0 ) {
                outPtr = addPointerToPackedOut( unpackedOutput, numElement * elementSz, NULL );
                unpackDoubleToOutPtr( inPtr, outPtr, numElement, myPackedItem.name, irodsProt );
                /* don't chk status. It could be a -ive int */
            }
            else {
                /* pointer to an array of pointers */
                for ( i = 0; i < numPointer; i++ ) {
                    outPtr = pointerArray[i] = malloc( numElement * elementSz );
                    status = unpackDoubleToOutPtr( inPtr, outPtr, numElement * elementSz, myPackedItem.name, irodsProt );
                    if ( status < 0 ) {
                        return status;
                    }
                }
        ...
        case PACK_STRUCT_TYPE: {
            ...
            if ( myDim == 0 ) {
                ...
                status = unpackChildStruct( inPtr, subPackedOutput, myPackedItem, myPackTable, numElement, irodsProt, nullptr, peer_version );
        ...
        default:
            rodsLog( LOG_ERROR, "unpackPointerItem: Unknown type %d - %s ", myTypeNum, myPackedItem.name );
            return SYS_PACK_INSTRUCT_FORMAT_ERR;
        }
    ...
    int unpackNatStringToOutPtr( const void *&inPtr, void *&outPtr, int maxStrLen )
    {
        ...
        if ( inPtr == NULL ) {
            rodsLog( LOG_ERROR, "unpackNatStringToOutPtr: NULL inPtr" );
            return SYS_PACK_INSTRUCT_FORMAT_ERR;
        }
        myStrlen = strlen( ( const char* )inPtr );
    ...
    {
        if (!inPtr) {
            rodsLog( LOG_ERROR, "unpackXmlStringToOutPtr: NULL inPtr" );
            return SYS_PACK_INSTRUCT_FORMAT_ERR;
        }
        ...
        char *myStrPtr;
        int myStrlen = xmlStrToStr((const char*) inPtr, origStrLen, myStrPtr, peer_version);
    ...
    int getAllocLenForStr( const packItem_t &myPackedItem, const void *inPtr, int numStr, int maxStrLen )
    {
        ...
        if ( numStr <= 1 ) {
            ...
            if ( maxStrLen < 0 ) {
                rodsLog( LOG_ERROR, "getAllocLenForStr: maxStrLen < 0 with numStr > 1 for %s", myPackedItem.name );
                return SYS_PACK_INSTRUCT_FORMAT_ERR;
            }
    ...
    packNopackPointer( void *inPtr, packedOutput_t &packedOutput, int len,
        ...
        status = packInt( intPtr, packedOutput, 1, name, irodsProt );
...
int packStruct(const void *inStruct,
               ...
               irodsProt_t irodsProt)
{
    if (!inStruct || !packedResult || !packInstName) {
        rodsLog( LOG_ERROR, "packStruct: Input error. One of the input is NULL" );
        return USER_PACKSTRUCT_INPUT_ERR;
    }
    ...
    int status = packChildStruct(inStruct, packedOutput, rootPackedItem,
                                 myPackTable, 1, packFlag, irodsProt, nullptr, std::nullopt);
                 ...
                 irodsProt_t irodsProt)
{
    if ( inPackedStr == NULL || packInstName == NULL ) {
        rodsLog( LOG_ERROR, "unpackStruct: Input error. One of the input is NULL" );
        return USER_PACKSTRUCT_INPUT_ERR;
    }
    ...
    int status = unpackChildStruct(inPackedStr, unpackedOutput, rootPackedItem,
                                   myPackTable, 1, irodsProt, nullptr, std::nullopt);
    free( rootPackedItem.name );
                ...
                const char* peer_version)
{
    ...
    std::optional<irods::version> peer_vers;
    if (peer_version) {
        peer_vers = to_version(peer_version);
    }
    ...
    packItem_t rootPackedItem{};
    rootPackedItem.name = strdup(packInstName);
    int status = packChildStruct(inStruct, packedOutput, rootPackedItem, myPackTable,
                                 1, packFlag, irodsProt, nullptr, peer_vers);
    free(rootPackedItem.name);
...
int unpack_struct(const void *inPackedStr,
                  void **outStruct,
                  const char *packInstName,
                  const packInstruct_t *myPackTable,
                  irodsProt_t irodsProt,
